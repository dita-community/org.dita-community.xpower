...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema"
    ...
    xmlns:ditamsg="http://dita-ot.sourceforge.net/ns/200704/ditamsg" exclude-result-prefixes="xs dita-ot dita2html ditamsg">
    ...
    <xsl:param name="BREADCRUMBS" select="'yes'"/>
    <!-- "no" and "yes" are valid values; non-'yes' is ignored -->
    ...
    <!-- TEMPDIR -->
    <xsl:param name="TEMPDIR" select="$WORKDIR"/>
    ...
    <xsl:variable name="mapFilename" select="concat('file:/', $TEMPDIR, '\ditamapref.xml')"/>
    <xsl:variable name="mapFilepath" select="replace($mapFilename, '\\', '/')"/>
    <xsl:variable name="mapFile" select="document($mapFilepath)" as="node()"/>
    ...
    <xsl:variable name="passthrough-attrs" as="element()*" select="$FILTERDOC/val/prop[@action = 'passthrough']"/>
    ...
    <xsl:template match="*[contains(@class, ' topic/topic ')]/*[contains(@class, ' topic/title ')]">
        ...
        <xsl:element name="h{$headinglevel}">
            ...
            <!--HSC create id of a heading in an HTML file
                Here we create the id of any html element that corresponds
                to a title. For the glossary, our @id may be gls_DCI which
                we put to the title of the appropriate HTML h2 construct
                in the glossary

                Currently the associated xref contatenates glsgrp__ to gls_DCI
                which breaks the link
            -->
            <xsl:attribute name="id">
                <!--HSX had to change to find glossary entries [MyDita#chmprocess].
          <xsl:apply-templates select="." mode="return-aria-label-id"/>
          -->
                <xsl:call-template name="setidaname"/>
            </xsl:attribute>

            <!--HSX here the title comes, glossentry has class topic/title but we
              do not want this to have the separator line.
      -->

            <xsl:choose>
                <xsl:when test="($headinglevel &lt; 7) and not(contains(../name(), 'glossentry'))">
                    <xsl:message>
                        <xsl:text>BREADCRUMS = </xsl:text>
                        <xsl:value-of select="$BREADCRUMBS"/>
                        <xsl:text>&#xA;ShitHead01:</xsl:text>
                        <xsl:value-of select="../name()"/>
                    </xsl:message>
                    <hr noshade="1" color="#1280FF" size="{7 - $headinglevel}"/>
                    <p style="margin-top:-0.5em; font-size:14pt;">
                        <xsl:apply-templates/>
                    </p>
                </xsl:when>
                <xsl:otherwise>
    ...
    <!-- paragraphs -->
    <!--HSX check outputclass for marginalia -->
    ...
    <xsl:template match="*" mode="process.note.common-processing">
        ...
        <div class="{$type}">
            ...
            <!--HSX create horizontal separator -->
            <hr noshade="1" color="#80E0E0" size="2"/>
            ...
            <xsl:apply-templates/>
            <!--HSX create horizontal note separator -->
            <hr noshade="1" color="#80E0E0" size="2"/>
    ...
    <xsl:template match="*[contains(@class, ' topic/itemgroup ')]" name="topic.itemgroup">
       ...
       Considering using div instead of span, with a default inline CSS style. -->
        <xsl:choose>
            <xsl:when
                test="
                    *[contains(@class, ' ditaot-d/ditaval-startprop ')]/revprop |
    ...
    <xsl:template name="getMatchingTarget" as="element()?">
        ...
        <xsl:variable name="glossentries" select="$m_entry-file-contents/descendant-or-self::*[contains(@class, ' glossentry/glossentry ')]"
            as="element()*"/>
        <xsl:choose>
    ...
    <xsl:template match="*" mode="getMatchingAcronym">
        ...
        <xsl:choose>
            <xsl:when test="exists($m_matched-target)">
                ...
                <xsl:choose>
                    <xsl:when
                        test="$glossentry//*[contains(@class, ' glossentry/glossStatus ')][@value = 'preferred'][1]/preceding-sibling::*[contains(@class, ' glossentry/glossAcronym ') or contains(@class, ' glossentry/glossAbbreviation ')][normalize-space(.) != '']">
                        <xsl:apply-templates
                            select="$glossentry//*[contains(@class, ' glossentry/glossStatus ')][@value = 'preferred'][1]/preceding-sibling::*[contains(@class, ' glossentry/glossAcronym ') or contains(@class, ' glossentry/glossAbbreviation ')][normalize-space(.) != '']"
                            mode="dita-ot:text-only"/>
                    </xsl:when>
                    <xsl:when
                        test="$glossentry//*[contains(@class, ' glossentry/glossStatus ')][@value != 'prohibited' and @value != 'obsolete'][1]/preceding-sibling::*[contains(@class, ' glossentry/glossAcronym ') or contains(@class, ' glossentry/glossAbbreviation ')][normalize-space(.) != '']">
                        <xsl:apply-templates
                            select="$glossentry//*[contains(@class, ' glossentry/glossStatus ')][@value != 'prohibited' and @value != 'obsolete'][1]/preceding-sibling::*[contains(@class, ' glossentry/glossAcronym ') or contains(@class, ' glossentry/glossAbbreviation ')][normalize-space(.) != '']"
                            mode="dita-ot:text-only"/>
                    </xsl:when>
                    <xsl:when
                        test="$glossentry//*[contains(@class, ' glossentry/glossAlt ')][1]/*[contains(@class, ' glossentry/glossAcronym ') or contains(@class, ' glossentry/glossAbbreviation ')][not(following-sibling::glossStatus)][normalize-space(.) != '']">
                        <xsl:apply-templates
                            select="$glossentry//*[contains(@class, ' glossentry/glossAlt ')][1]/*[contains(@class, ' glossentry/glossAcronym ') or contains(@class, ' glossentry/glossAbbreviation ')][count(following-sibling::glossStatus) = 0][normalize-space(.) != '']"
                            mode="dita-ot:text-only"/>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:apply-templates select="$glossentry/*[contains(@class, ' glossentry/glossterm ')]" mode="dita-ot:text-only"/>
      ...
      -->
                ...
                <xsl:variable name="displaytext">
                    <xsl:choose>
                        ...
                        <xsl:when
                            test="exists(ancestor::*[contains(@class, ' topic/copyright ')]) or generate-id(.) = generate-id(key('keyref', @keyref)[1])">
                ...
                <a>
                    ...
                    <xsl:apply-templates select="." mode="add-desc-as-hoverhelp">
                        <xsl:with-param name="hovertext" select="$hovertext"> </xsl:with-param>
                    </xsl:apply-templates>
    ...
    <xsl:function name="dita-ot:generate-html-id" as="xs:string">
        ...
        <xsl:sequence
            select="
                ...
                then
                    dita-ot:get-prefixed-id($element, $element/@id)
                else
                    generate-id($element)"
        />
    </xsl:function>
    ...
    <xsl:template name="gen-toc-id"> </xsl:template>
       ...
       combine user output class with element default, giving priority to the user value. -->
        <xsl:if test="string-length(normalize-space(concat($outputclass-attribute, $using-output-class, $ancestry))) > 0">
            <xsl:attribute name="class">
                ...
                <xsl:if
                    test="
                        string-length(normalize-space($ancestry)) > 0 and
                        string-length(normalize-space($using-output-class)) > 0">
                    <xsl:text> </xsl:text>
                </xsl:if>
                <xsl:value-of select="normalize-space($using-output-class)"/>
                <xsl:if
                    test="
                        string-length(normalize-space(concat($ancestry, $using-output-class))) > 0 and
                        string-length(normalize-space($outputclass-attribute)) > 0">
                    <xsl:text> </xsl:text>
                </xsl:if>
        ...
        <span style="background-color: yellow;">
            <span style="font-weight: bold">
                ...
                <xsl:for-each select="ancestor-or-self::*">
                    <xsl:text>/</xsl:text>
                    <xsl:value-of select="name()"/>
                </xsl:for-each> {"<xsl:value-of select="@class"/>"}<xsl:text>) </xsl:text>
            </span>
            <xsl:apply-templates/>
            <span style="font-weight: bold">
                <xsl:text> (</xsl:text>
    ...
    <xsl:template
        ...
        <xsl:element name="{$headLevel}">
            ...
            <!--HSX possible CSS settings for hr
      hr {
    display: block;
    margin-top: 0.5em;
    margin-bottom: 0.5em;
    ...
    -->
            ...
            <hr noshade="1" color="#C0C0C0" size="3"/>
            <p style="margin-top:-0.5em; font-size:12pt;">
                <xsl:apply-templates/>
            </p>
        </xsl:element>
    </xsl:template>
    ...
    <!--set string to 'yes' to turn on trace -->
    ...
    <!-- find xref which refers to footnote. -->
 ...
 +-->
    ...
    <xsl:template match="*[contains(@class, ' topic/fig ')]/*[contains(@class, ' topic/desc ')]" mode="get-output-class"
        >figdesc</xsl:template>
    ...
    <!--HSX put line over chapter title -->
    <!--
  <hr noshade="1" color="#1280FF" size="5"><p style="margin-top:-0.3em; font-size:16pt;">TITLE</p>
  -->
    <xsl:template name="generateChapterTitle">
        ...
        <title>
            ...
            <xsl:variable name="ditamaintitle">
                <xsl:apply-templates select="/dita/*[contains(@class, ' topic/topic ')][1]/*[contains(@class, ' topic/title ')]"
                    mode="text-only"/>
            </xsl:variable>
            <xsl:choose>
    ...
    <xsl:template match="*" mode="chapterBody">
        <body>
            ...
            <xsl:call-template name="setaname"/>
            <!-- For HTML4 compatibility, if needed -->
            <xsl:value-of select="$newline"/>
            ...
            <!--HSX add prev/next buttons -->
            <xsl:variable name="xtrf" select="@xtrf"/>
            <xsl:variable name="myPos" as="xs:integer">
                <xsl:for-each select="$mapFile/hrefs/*">
                    <xsl:if test="ends-with($xtrf, normalize-space(current()))">
            ...
            <xsl:message>
                <xsl:text>Shit-Mapref=gen next:prev = </xsl:text>

                <xsl:text>&#xA;Prev:</xsl:text>
                <xsl:call-template name="get-file-name">
                    <xsl:with-param name="file-path"
                        select="
                            $mapFile/hrefs/href[if ($myPos &gt; 1) then
                                ($myPos - 1)
                            else
                                $myPos]"
                    />
                ...
                <xsl:text>&#xA;This:</xsl:text>
                <xsl:call-template name="get-file-name">
                    <xsl:with-param name="file-path" select="$mapFile/hrefs/href[$myPos]"/>
                </xsl:call-template>
                <xsl:text>&#xA;Next:</xsl:text>
                <xsl:call-template name="get-file-name">
                    <xsl:with-param name="file-path"
                        select="
                            $mapFile/hrefs/href[if ($myPos &lt; count($mapFile/hrefs/href)) then
                                ($myPos + 1)
                            else
                                $myPos]"
                    />
            ...
            <xsl:if test="$myPos &gt; 1">
                <xsl:variable name="prev">
                    <xsl:call-template name="get-file-name">
                        <xsl:with-param name="file-path" select="$mapFile/hrefs/href[$myPos - 1]"/>
                    </xsl:call-template>
                </xsl:variable>
                <div>
                    <a class="link" href="{concat(normalize-space(substring-before($prev, '.')), $OUTEXT)}">PREV</a>
                </div>
            </xsl:if>

            <xsl:if test="$myPos &lt; count($mapFile/hrefs/href)">
                ...
                <div>
                    <a class="link" href="{concat(normalize-space(substring-before($next, '.')), $OUTEXT)}">NEXT</a>
                </div>
            </xsl:if>
            <!--
      <div><a class="link" href="Glossary.html">next</a></div></div>
      -->
    ...
    <!-- Override this template to add any standard attributes to
       ...
       element of the doc. -->
    <xsl:template match="*" mode="addAttributesToBody"> </xsl:template>
    ...
    <!--HSC ExtractFileName
        strips the filename through rekursive call
        which eliminates the path -->
    ...
    <xsl:template match="*" mode="find-keyref-target">
        ...
        <xsl:choose>
            ...
            <xsl:when test="contains($target, '#')">
                <xsl:value-of
                    select="concat($PATH2PROJ, substring-before(substring-before($target, '#'), '.'), $OUTEXT, '#', substring-after($target, '#'))"
                />
            </xsl:when>
        ...
        <xsl:choose>
            <xsl:when test="$displaytext = '' and $keys != ''">
                ...
                <xsl:if test="not($target = '' or contains($target, '://'))">
                    <xsl:value-of
                        select="(document(concat($WORKDIR, $PATH2PROJ, $target))//*[contains(@class, ' topic/title ')][normalize-space(.) != ''])[1]"
                    />
                </xsl:if>
            </xsl:when>
