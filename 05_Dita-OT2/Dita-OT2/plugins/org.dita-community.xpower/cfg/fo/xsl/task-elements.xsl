...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format"
    xmlns:dita2xslfo="http://dita-ot.sourceforge.net/ns/200910/dita2xslfo" xmlns:opentopic="http://www.idiominc.com/opentopic"
    xmlns:opentopic-index="http://www.idiominc.com/opentopic/index" exclude-result-prefixes="opentopic opentopic-index dita2xslfo"
    version="2.0">
    ...
    <xsl:template match="*[contains(@class, ' task/task ')]" mode="processTopic" name="processTask">
        <xsl:param name="mode" select="0"/>
        <xsl:if test="contains($unittest, 'chkproc')">
            <xsl:message>
                <xsl:text>ProcTask[</xsl:text>
    ...
    <xsl:template match="*[contains(@class, ' task/context ')]">
        <!-- this fo:block is already the second fo:block, the caller wraps another one -->
        <fo:block xsl:use-attribute-sets="context">
            <xsl:call-template name="commonattributes"/>
            <!--HSX the following apply-templates dita2xslfo:task-heading delivers another fo:block 
               That block uses attribute-sets task.title and overrides anything set in
               the attribute-sets "context". So "context" should only set things common
               to title and context__content (e.g. margins or basic font).
            -->
            <xsl:apply-templates select="." mode="dita2xslfo:task-heading">
                <xsl:with-param name="use-label">
                    <xsl:apply-templates select="." mode="dita2xslfo:retrieve-task-heading">
    ...
    <xsl:template match="*[contains(@class, ' task/steps ')]">
        <!--HSC [DtPrt#12.8.1] Add a sentence prior to the steps to indicate the maximum number of steps -->
        <xsl:variable name="actual-step-count" select="count(*[contains(@class, ' task/step ')])"/>
        <fo:block xsl:use-attribute-sets="actualsteps">
            <xsl:if test="$actual-step-count > 999">
                <xsl:text>This task has </xsl:text>
                <xsl:value-of select="count(*[contains(@class, ' task/step ')])"/>
                <xsl:text> steps (min. no of steps to show this can be set in task-elements.xsl).</xsl:text>
            </xsl:if>
        </fo:block>
            ...
            <fo:list-item-label xsl:use-attribute-sets="steps.step__label">
                <fo:block xsl:use-attribute-sets="steps.step__label__content">
                    ...
                    <xsl:if
                        ...
                        <xsl:call-template name="getVariable">
                            <!--HSC to create separate numbering for steps, we follow [DtPrt#12.7]
        ...
        <fo:list-item xsl:use-attribute-sets="substeps.substep">
            <fo:list-item-label xsl:use-attribute-sets="substeps.substep__label">
                <fo:block xsl:use-attribute-sets="substeps.substep__label__content">
                    ...
                    <!--HSC [DtPrt#12.7] suggests to use format 'i'
            <xsl:number format="a) "/> -->
                    <xsl:number format="i. "/>
                </fo:block>
            </fo:list-item-label>
