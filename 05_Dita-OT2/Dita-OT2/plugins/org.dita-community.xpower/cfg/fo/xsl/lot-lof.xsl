...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <fo:block xsl:use-attribute-sets="__lotf__indent">
      <fo:block xsl:use-attribute-sets="__lotf__content">
        <fo:basic-link xsl:use-attribute-sets="__toc__link">
            ...
            <xsl:call-template name="getVariable">
              ...
              <xsl:with-param name="params">
                <number>
                  ...
                  <!--HSC [DtPrt#18.8] allows to add the chapter prefix to the LOT/LOF entries -->
          ...
          <!--HSC to add chapters to page numbers [DtPrt#18.6] suggests
                  the changes. Here I'm using just the other exampl as a
                  full block -->
              
          <xsl:choose>
            <xsl:when test="contains($PageLayout, 'prefix_chapter')">
  ...
  <!--HSC Here we meet any figure to be listed in the list.of.figures -->
    ...
    <fo:block xsl:use-attribute-sets="__lotf__indent">
      <fo:block xsl:use-attribute-sets="__lotf__content">
        <fo:basic-link xsl:use-attribute-sets="__toc__link">
          ...
          <xsl:variable name="prefixCh">
              <xsl:call-template name="getChapterPrefix" />
          </xsl:variable>
          
          <!--HSC create "Figures" entry -->
          <fo:inline xsl:use-attribute-sets="__lotf__title">

            <!--HSC The Figure.title is actually the 'label' e.g. "Figure <num>:" -->
            <xsl:call-template name="getVariable">
          ...
          <xsl:choose>
            <xsl:when test="contains($PageLayout, 'prefix_chapter')">
              <fo:inline xsl:use-attribute-sets="__lotf__page-number">
                ...
                <fo:leader xsl:use-attribute-sets="__lotf__leader"/>
                <xsl:text>&#xA0;&#xA0;</xsl:text>
                <xsl:if test="string-length(normalize-space($prefixCh)) &gt; 0">
                    <xsl:value-of select="$prefixCh"/>
                    <xsl:text>-</xsl:text>
                </xsl:if>
                <fo:page-number-citation>
