...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema"
    ...
    <xsl:template match="*" mode="commonTopicProcessing">
        ...
        <xsl:if test="not(contains(@outputclass, 'noprint'))">
            ...
            <!--HSX We want to use a heading from a chapter level
                which is smaller or equal $topic.Header.numLevel

                Therefore we collect all potential headers from ancestors.
            -->
            ...
            <!--HSX we determine the heading level we like to see in the heading/footer -->
            ...
            <!--HSX We get the node relevant to be the heading marker -->
            <xsl:variable name="markerNode" as="node()">
                <xsl:variable name="titleText">
                    <xsl:value-of select="$markerNodes/*[$mrkidx]/title"/>                    
                </xsl:variable>
                <xsl:choose>
                    <xsl:when test="string-length($titleText) &gt; 0">
                        <!-- safe choice but requires parsing // later
                        -->
                        <xsl:copy-of select="ancestor-or-self::*[@id = $markerNodes/*[$mrkidx]/@id]"/>
                        <!--
                        <xsl:copy-of select="(//*[@id = $markerNodes/*[$mrkidx]/@id])[1]"/>
                        -->
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:variable name="prevNode" as="node()?">
                            <!--HSX get the previous topic, cannot simply be done bye
                                    preceding-sibling:: because the previous topic is not
                                    visible in the present scope.
                            -->
                            <!--HSX template based selection - safe of multiple matching ids
                            <xsl:apply-templates select="/" mode="anytopic">
                                <xsl:with-param name="myId" select="$markerNodes/*[$mrkidx]/@id" tunnel="yes"/>
                            </xsl:apply-templates>
                            -->
                            <!--HSX one-line solution, allows later direct use of the result
                            --> 
                            <xsl:copy-of select="//*[@id = $markerNodes/*[$mrkidx]/@id]/preceding-sibling::*[contains(@class, ' topic/topic ')][1]"/>
                        </xsl:variable>
                        <xsl:copy-of select="$prevNode"/>
                    </xsl:otherwise>
                </xsl:choose>                
            </xsl:variable>
            ...
            <!--HSX since we never need to assume that we never
                put an empty-title topic as first child of a
                topic, we may always assume a preceding-sibling of the
                selected marker            
            -->
            ...
            <!--HSX create marker from the latest non-empty title -->
            <xsl:variable name="head2marker" as="node()">
                <fo:marker marker-class-name="current-header">
                    <xsl:apply-templates select="//*[@id = $markerNode/@id]/title" mode="getNumTitle">
                        <xsl:with-param name="navTitle" select="$navtitle"/>
                    </xsl:apply-templates>
                    <!--
                    <xsl:apply-templates select="ancestor-or-self::*[@id = $markerNode/@id]/title" mode="getNumTitle">
    ...
    <!--HSX3 Templates to return the preceding sibling of a node with id = myId -->
    <xsl:template match="*[contains(@class, ' topic/topic ')] " mode="anytopic" priority="10">
        <xsl:param name="myId" tunnel="yes"/>
        <xsl:choose>
            <xsl:when test="@id = $myId">
                <xsl:message>
                    <xsl:text>&#xA;FOUND: topic/topic[</xsl:text>
                    <xsl:value-of select="$myId"/>
                    <xsl:text>] = </xsl:text>
                    <xsl:value-of select="preceding-sibling::*[contains(@class, ' topic/topic ')][1]/@id"/>
                    <xsl:text> - </xsl:text>
                    <xsl:value-of select="preceding-sibling::*[contains(@class, ' topic/topic ')][1]/title"/>
                </xsl:message>
                <xsl:copy-of select="preceding-sibling::*[contains(@class, ' topic/topic ')][1]"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:message>
                    <xsl:text>&#xA;SEEK: topic/topic[</xsl:text>
                    <xsl:value-of select="$myId"/>
                    <xsl:text>] = </xsl:text>
                    <xsl:value-of select="preceding-sibling::*[contains(@class, ' topic/topic ')][1]/@id"/>
                </xsl:message>
                <xsl:apply-templates mode="anytopic"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>

    <xsl:template match="text()" mode="anytopic"/>

    <xsl:template match="*" mode="anytopic" priority="-1">
        <xsl:apply-templates mode="anytopic"/>
    </xsl:template>
        ...
        <xsl:choose>
            ...
            <xsl:when
                ...
                <xsl:choose>
                    ...
                    <xsl:when
                        test="
                            not(ancestor::*[contains(@class, ' topic/topic ')]) and
            ...
            <!-- topicSimple topicAppendix topicAppendices -->
            <xsl:otherwise>
    ...
    <!--HSX3 - only as reference - never use this .. this is the basic call to get the titles -->
    <xsl:template match="*" mode="insertTopicHeaderMarkerX">
        <xsl:param name="marker-class-name" select="string('current-header')"/>
    ...
    <xsl:template match="*" mode="insertTopicHeaderMarker">
        ...
        <!--HSX3 the content we produce in the marker will be the text
                 that is shown in the TOC and in the heading of the pages.
        -->
        <xsl:choose>
            <xsl:when test="$marker-class-name = 'NOCLASS'">
                <xsl:apply-templates select="." mode="insertTopicHeaderMarkerContents">
                    <xsl:with-param name="sidecol" select="$sidecol"/>
                    <xsl:with-param name="navTitle" select="$navTitle"/>
            ...
            <xsl:otherwise>
                <fo:marker marker-class-name="{$marker-class-name}">
                    <!--HSX3 This call should give back the numbered title text -->
                    <xsl:apply-templates select="." mode="insertTopicHeaderMarkerContents">
                        <xsl:with-param name="sidecol" select="$sidecol"/>
                        <xsl:with-param name="navTitle" select="$navTitle"/>
                        <xsl:with-param name="isFromNav" select="$isFromNav"/>
    ...
    <!--HSX3 we are in any topic ... so we do getNumTitle to our title-children -->
    <xsl:template match="*[contains(@class, ' topic/topic ')]" mode="insertTopicHeaderMarkerContents">
        ...
        <xsl:apply-templates select="*[contains(@class, ' topic/title ')]" mode="getNumTitle">
            <xsl:with-param name="sidecol" as="xs:boolean" select="false()"/>
            <xsl:with-param name="navTitle" as="xs:string" select="string('')"/>
            <xsl:with-param name="isFromNav" as="xs:boolean" select="false()"/>
        </xsl:apply-templates>
    </xsl:template>

    <!--HSX3 we are in title ... so we do getNumTitle with our own node -->
    ...
    <xsl:template name="processTopicChapter">
        ...
        <xsl:variable name="head1marker" as="node()">
            <!--HSC Visit all title children of topic (e.g. concept) typically there's only one -->
            <!--HSD I know that we are in concept here !!!! -->
            <fo:marker marker-class-name="current-header">
                <!--HSC Visit all title children of topic (e.g. concept) typically there's only one -->
                ...
                <fo:block xsl:use-attribute-sets="topic">
                    ...
                    <xsl:choose>
                        <xsl:when test="$chapterLayout = 'BASIC'">
                            <xsl:apply-templates
                                select="
                                    *[not(contains(@class, ' topic/topic ') or
    ...
    <xsl:template name="putFO">
        ...
        <xsl:element name="{name()}">
            <xsl:for-each select="@*[not(contains(name(), 'idx'))]">
            ...
            <fo:flow flow-name="xsl-region-body">
                <fo:block xsl:use-attribute-sets="topic">
                    ...
                    <fo:block xsl:use-attribute-sets="topic.title">
                        ...
                        <xsl:apply-templates select="." mode="insertTopicHeaderMarker">
                            <xsl:with-param name="setMarker" select="false()"/>
                        </xsl:apply-templates>
        ...
        <fo:page-sequence master-reference="body-sequence" xsl:use-attribute-sets="page-sequence.notice">
            ...
            <fo:flow flow-name="xsl-region-body">
                <fo:block xsl:use-attribute-sets="topic">
                    ...
                    <fo:block xsl:use-attribute-sets="topic.title">
                        <xsl:call-template name="pullPrologIndexTerms"/>
                        <!--HSX3 create title but no markers -->
                        <xsl:apply-templates select="." mode="insertTopicHeaderMarker">
                            <xsl:with-param name="marker-class-name" select="'NOCLASS'"/>
                        </xsl:apply-templates>
                        <!--HSX3
                        <xsl:for-each select="child::*[contains(@class, ' topic/title ')]">
                            <xsl:apply-templates select="." mode="getNumTitle">
    ...
    <!--HSX3:topicFrontMatter
             processFrontMatterTopic came in with Dita-OT 2.3.1
             It is called from line 535 where a new when-case is
             made exactly for this purpose. You can avoid calling this
             by changing line 535 to catch nothing
    -->
    ...
    <xsl:template
        ...
        <!--HSX3 allow selection of HSX method (test=1) or DITA-OT 2.3.1 (test=0) -->
        <xsl:choose>
            <xsl:when test="0">
                <fo:inline>
    ...
    <xsl:template match="*" mode="processTopicTitle">
        ...
        <xsl:choose>
            <!-- next condition shall never occur, except author puts an empty title
                 as first sub-chapter (which is stupid to do)   
            --> 
            <xsl:when test="$txtLen = 0">
                <fo:marker marker-class-name="current-header">HSXT:NO_HEADING</fo:marker>
            </xsl:when>
            <xsl:when test="(count($chkGlossary) &gt; 0) and not(contains($mainTitles, 'mainGlossTitle'))"> </xsl:when>
            ...
            <xsl:otherwise>
                ...
                <fo:block>
                    ...
                    <fo:block>
                        ...
                        <xsl:if test="$level = 2">
                            <xsl:apply-templates select="." mode="insertTopicHeaderMarker">
                                <xsl:with-param name="marker-class-name" select="current-h2"/>
                            </xsl:apply-templates>
                            <!--HSX3
    ...
    <xsl:template match="*[contains(@class, ' topic/section ')]/*[contains(@class, ' topic/title ')]">
        ...
        <xsl:choose>
            <xsl:when test="contains(../@outputclass, 'Block.') or contains(../@outputclass, 'mrg')">
                ...
                <fo:float float="start" clear="both" xsl:use-attribute-sets="p_marginalia_heading">
                    ...
                    <fo:block-container padding-top="1pt">
                        ...
                        <fo:block>
                            <!--HSX3 do no apply markers => select="''" -->
                            <xsl:apply-templates select="." mode="insertTopicHeaderMarker">
                                <xsl:with-param name="marker-class-name" select="'NOCLASS'"/>
                            </xsl:apply-templates>
                            <!--HSX3
            ...
            <xsl:otherwise>
                <!--HSX currently same as outputclass contains "page" -->
                <fo:block xsl:use-attribute-sets="section.title.page">
                    <xsl:call-template name="commonattributes"/>
                    <fo:block xsl:use-attribute-sets="section.title.page.text">
                        <xsl:apply-templates select="." mode="insertTopicHeaderMarker">
                                ...
                                <xsl:choose>
                                    <xsl:when test="contains($EnumerationMode, 'fig_numrestart')">
                                        <xsl:choose>
                                            <xsl:when
                                                test="
                                                    count(ancestor-or-self::*[contains(@class, ' topic/topic')][position() = last()]
                                                    [count(preceding-sibling::*[contains(@class, ' topic/topic')]) &gt; 0])">
                                                <xsl:value-of
                                                    select="
                                                        count(./preceding::*[contains(@class, ' topic/fig ')]
                                            ...
                                            <xsl:otherwise>
                                                <xsl:value-of
                                                    select="
                                                        count(./preceding::*[contains(@class, ' topic/fig ')]
                                                        [child::*[contains(@class, ' topic/title ')]]
                                                        [ancestor-or-self::*[contains(@class, ' topic/topic')][position() = last()]]) + 1"
                                                />
                                            </xsl:otherwise>
                                        </xsl:choose>
                                    </xsl:when>
    ...
    <!--HSX3 This template is not used - it is introduced in DITA-OT 2.3.1 but
             I have a better solution above, I keep it here for synchronization
             It is actually invoked for the simple version in the original DITA-OT
             using ...
                 <xsl:apply-templates select="." mode="fig.title-number"/>
             ... but this is not sufficient for the mode that the xpower plugin provides
    -->
    ...
    <xsl:template name="getAppendixNo">
        ...
        <xsl:variable name="normPrefix">
            <xsl:number count="*[contains(@class, ' topic/topic ')][@id = //*/appendix/@id | //*/(topicref | topicset)/@id]"
                level="multiple" format="A.1"/>
        </xsl:variable>

        <xsl:variable name="apSuffix">
            <xsl:number count="*[contains(@class, ' topic/topic ')][@id = //*/appendiy/@id | //*/(topicref | topicset)/@id]"
        ...
        <xsl:variable name="apPrefix">
            <xsl:number
                value="
                    count(preceding::*/@outputclass[contains(., 'appendix')]) +
    ...
    <xsl:template name="getTitleNumber">
        ...
        <xsl:choose>
            ...
            <xsl:when
                test="
                    not(count($level) &gt; $topicTitle.numLevel) and
            ...
            <xsl:when
                ...
                <xsl:for-each select="$level">
                    ...
                    <xsl:value-of
                        select="
                            count(preceding-sibling::*[contains(@class, ' topic/topic ')][@id = //*/chapter/@id]) +
            ...
            <xsl:when
                ...
                <xsl:choose>
                    ...
                    <xsl:when test="$topicType = 'topicAppendix'">
                        <xsl:number count="*[contains(@class, ' topic/topic ')][@id = //*/chapter/@id | //*/(topicref | topicset)/@id]"
                            level="multiple" format="A.1"/>
                    </xsl:when>
    ...
    <xsl:template
        match="
            *[contains(@class, ' topic/titlealts ')]/*[contains(@class, ' topic/searchtitle ')] |
    ...
    <xsl:template match="*[contains(@class, ' topic/shortdesc ')]">
        <xsl:variable name="format-as-block" as="xs:boolean">
            <xsl:choose>
                ...
                <xsl:when
                    test="
                        preceding-sibling::*[contains(@class, ' topic/p ') or contains(@class, ' topic/dl ') or
                ...
                <xsl:when
                    test="
                        following-sibling::*[contains(@class, ' topic/p ') or contains(@class, ' topic/dl ') or
                        contains(@class, ' topic/fig ') or contains(@class, ' topic/lines ') or
                        contains(@class, ' topic/lq ') or contains(@class, ' topic/note ') or
                        contains(@class, ' topic/ol ') or contains(@class, ' topic/pre ') or
                        contains(@class, ' topic/simpletable ') or contains(@class, ' topic/sl ') or
    ...
    <xsl:template match="*[contains(@class, ' topic/topic ')]/*[contains(@class, ' topic/shortdesc ')]" priority="1">
        ...
        <xsl:if test="contains($PageLayout, 'print_shortdesc')">
            <xsl:choose>
                ...
                <xsl:when
                    test="
                        $topicType = ('topicChapter', 'topicAppendix') and
                        $chapterLayout != 'BASIC'"/>
                <!--   Normal processing         -->
                <xsl:otherwise>
                    <xsl:apply-templates select="." mode="format-shortdesc-as-block"/>
    ...
    <xsl:template
        ...
        <fo:block xsl:use-attribute-sets="section.title">
            ...
            <xsl:sequence
                select="
                    if (not(normalize-space($resolvedVariable)))
                    then
                        $spectitleValue
                    else
                        $resolvedVariable"
    ...
    <xsl:template match="*[contains(@class, ' topic/section ')]">
        ...
        <xsl:choose>
            <xsl:when test="title and (string-length(normalize-space(title)) &gt; 0)">
                <!--HSX the empty block avoids to join previous blocks on the page
                        by the keep-with-previous statement which is necessary for
                        keeping the sections components (line, title) together.

                        I had the situation that a ul list's list item was pulled to the next
                        page where a section started although there was plenty of
                        space for the list item on its page.
                -->
                <fo:block/>
    ...
    <xsl:template name="pullPrologIndexTerms">
        ...
        <xsl:apply-templates
            select="
                ancestor-or-self::*[contains(@class, ' topic/topic ')][1]/*[contains(@class, ' topic/prolog ')]
    ...
    <xsl:template name="pullPrologIndexTerms.end-range">
        ...
        <xsl:apply-templates
            select="
                ancestor-or-self::*[contains(@class, ' topic/topic ')][1]/
                *[contains(@class, ' topic/prolog ')]/
    ...
    <xsl:template name="condKeep">
        ...
        <xsl:if
            test="
                (($titleDistance &gt; 0) and ($titleDistance &lt; $maxKeepLines)) or
    ...
    <xsl:template match="*[contains(@class, ' topic/p ')]">
        ...
        <xsl:choose>
            <xsl:when
                test="
                    ancestor::*[contains(@class, ' topic/example ') or
            ...
            <xsl:when
                test="
                    ancestor::*[contains(@class, ' topic/sl ') or
            ...
            a section
            -->
            <xsl:when test="contains(preceding-sibling::*[1]/@outputclass, 'mrg')">
            ...
            <xsl:when test="contains(preceding-sibling::*[1]/@class, ' topic/title ')">
    ...
    <xsl:template match="*[contains(@class, ' topic/note ')]">
        ...
        <fo:table xsl:use-attribute-sets="note__table">
            ...
            <!--HSX Fixing problem of a note within a list
                    The list defines relative-align="baseline" which places the
                    content of the note text at the base of the image. We don't want
                    this, our block-progression shall be on the top of the image
                    and therefore we need to set the relative-align to "before"
            -->
            <fo:table-body relative-align="before">
                <fo:table-row>
                    <fo:table-cell xsl:use-attribute-sets="note__image__entry">
                        ...
                        <xsl:if test="not(contains(@outputclass, 'noImage')) and not($noteImagePath = '')">
                            <fo:block xsl:use-attribute-sets="note__text">
                                <fo:external-graphic src="url('{concat($artworkPrefix, $noteImagePath)}')" xsl:use-attribute-sets="image"/>
                            </fo:block>
                        </xsl:if>
                    </fo:table-cell>
    ...
    <xsl:template match="*[contains(@class, ' topic/fig ')]">
        ...
        <fo:block xsl:use-attribute-sets="fig">
            <xsl:call-template name="chkNewpage"/>
            <!--HSX chkNewpage replaces commonattributes -->
    ...
    <xsl:template match="*[contains(@class, ' topic/ph ')]">
        <fo:inline>
            <!-- xsl:use-attribute-sets="ph" -->
            <xsl:call-template name="commonattributes"/>
            <!--HSX allow some manual formatting, freedom to the author

                We are using "ph" as a joker that transports any
                attribute definition that fits into the context.
                You can do quite some nonsense here, so use this with
                responsibility.
            -->
            <xsl:for-each select="tokenize(@outputclass, ' ')">
    ...
    <xsl:template match="*[contains(@class, ' topic/image ')]">
        ...
        <xsl:variable name="equation_scale" as="xs:integer">
            <xsl:choose>
                <xsl:when test="parent::equation-block or parent::equation-inline">
                    <xsl:choose>
        ...
        <xsl:choose>
            <xsl:when test="not(@placement = 'inline')">
                ...
                <!--HSX we compute the width from the input
                    To respect odd aspect ration, we have the correction factors
                    to change the width if the aspect ratio of the image
                    does not match the page.
                -->
                <xsl:variable name="shrink" as="xs:double">
                    <xsl:choose>
                        <xsl:when test="contains(@outputclass, 'size=')">
                            <xsl:analyze-string select="@outputclass" regex="{'size=([0-9]+\.*[0-9]*)%'}">
                                <xsl:matching-substring>
                                    <xsl:value-of select="number(regex-group(1)) div 100"/>
                                </xsl:matching-substring>
                                <xsl:non-matching-substring>
                ...
                <xsl:variable name="sWidth">
                    <xsl:choose>
                        ...
                        <!--HSX expanse = page computes the maximum width and height to make the image
                                fit on the page
                        -->
                        <xsl:when
                            test="
                                (contains(@outputclass, 'page') and ($cntAncP = 0)) or
                                (ancestor::*[contains(name(), 'fig')]/@expanse = 'page')">

                            <xsl:if test="contains($unittest, 'chkexpanse')">
                                <xsl:message>
                                    <xsl:text>HSXT-Pagewide:</xsl:text>
                                    <xsl:value-of select="ancestor::*[contains(@class, ' topic/topic ')][1]/@outputclass"/>
                                    <xsl:text>&#xA;RawWidth:</xsl:text>
                            ...
                            <xsl:variable name="availWidth">
                                <xsl:choose>
                                    <xsl:when test="contains((ancestor::*[contains(@class, ' topic/topic ')][1])/@outputclass, 'landscape')">
                                        <xsl:call-template name="getPagewideMM">
                                            <xsl:with-param name="isLandscape" select="true()"/>
                                        </xsl:call-template>
                                    </xsl:when>
                                    <xsl:otherwise>
                                        <xsl:call-template name="getPagewideMM"/>
                            ...
                            <xsl:variable name="availHeight">
                                <xsl:choose>

                                    <xsl:when test="contains((ancestor::*[contains(@class, ' topic/topic ')][1])/@outputclass, 'landscape')">
                                        <xsl:call-template name="getPageheightMM">
                                            <xsl:with-param name="isLandscape" select="true()"/>
                                        </xsl:call-template>
                                    </xsl:when>
                                    <xsl:otherwise>
                            ...
                            <!--HSX
                                Here we do the page correction !!!
                                frameFactor is a number below 1 to reduce the picture if we
                                expect a heading that should be on the same page as the picture
                                We check the parents/title to see the requirement to reduce the
                                sice (frameFactor &lt; 1) to fit on the page
                            -->
                            <xsl:variable name="myNode" select="ancestor::*[contains(@class, ' topic/topic ')][1]" as="node()"/>

                            <xsl:variable name="frameFactor" as="xs:double">
                                <xsl:choose>
                                    <xsl:when test="string-length(normalize-space($myNode/title)) &gt; 0">
                                        <xsl:choose>
                                            <xsl:when test="exists($myNode/shortdesc)">
                                                <xsl:value-of select="0.82 * $shrink"/>
                                            </xsl:when>
                                            <xsl:otherwise>
                                                <xsl:value-of select="0.92 * $shrink"/>
                                            </xsl:otherwise>
                                        </xsl:choose>
                                    </xsl:when>
                                    <xsl:otherwise>
                                        <xsl:value-of select="$shrink"/>
                                    </xsl:otherwise>
                                </xsl:choose>
                            </xsl:variable>
                            <xsl:if test="contains($unittest, 'chkexpanse')">
                                <xsl:message>
                                    <xsl:text>FrameFactor: </xsl:text>
                                    <xsl:value-of select="$frameFactor"/>
                                    <xsl:text>&#xA;availWidth = </xsl:text>
                                    <xsl:value-of select="$availWidth"/>
                                    <xsl:text>&#xA;availHeight = </xsl:text>
                                    <xsl:value-of select="$availHeight"/>
                                </xsl:message>
                            </xsl:if>

                            <xsl:choose>
                                <!--HSX check whether we could not size from svgsize.xml -->
                                <xsl:when test="($rawHeight &gt; 99998)">
                                    <xsl:value-of select="-1"/>
                                </xsl:when>

                                <xsl:when test="($rawWidth div $availWidth) &gt; ($rawHeight div $availHeight)">
                                    <xsl:value-of select="concat(($frameFactor * $availWidth), 'mm')"/>
                                </xsl:when>
                                <xsl:otherwise>
                                    <xsl:value-of select="concat(($frameFactor * $availHeight * $rawWidth div $rawHeight), 'mm')"/>
                                </xsl:otherwise>
                            </xsl:choose>
                        </xsl:when>
                ...
                <xsl:choose>
                    <!--HSX check whether we couldn't find svgsize.xml or an
                            entry was missing. If YES ... give out a warning text
                            with a link to the explanation.
                    -->
                    <xsl:when test="string($sWidth) = '-1'">
                        <fo:block font-size="16pt" font-family="sans-serif" color="red">
                            <xsl:text>Error: Run </xsl:text>
                            <fo:inline color="blue">svgExtractSize.xsl </fo:inline>
                            <fo:inline>to compute the image size of SVG files !</fo:inline>
                        </fo:block>
                        ...
                        <fo:block>
                            <fo:basic-link xsl:use-attribute-sets="xref">
                                <xsl:attribute name="font-size" select="'16pt'"/>
                                <xsl:attribute name="external-destination">
                                    <xsl:value-of select="concat($DocuPath, '\dev\ref\stb\xpwr\xpwr_expwidth.stb')"/>
                                </xsl:attribute>
                                <xsl:value-of select="'&#x2192; [xpwr#expwidth]'"/>
                            </fo:basic-link>
                        </fo:block>
                        <fo:block font-size="16pt" font-family="sans-serif">
                            <xsl:text>&#xA;&#xA0;&#xA;&#xA0;</xsl:text>
                    ...
                    <!--HSX All sizes OK - print the image -->
                    <xsl:otherwise>
    ...
    <xsl:function name="opentopic-func:isAbsolute" as="xs:boolean">
        ...
        <xsl:sequence
            select="
                some $prefix in ('/', 'file:')
                    satisfies starts-with($uri, $prefix) or
                    contains($uri, '://')
                    or contains($uri, ':/') or contains($uri, ':\')"/>
        <!--HSC added Windows fullpath, :// becomes obsolete but I left it -->
    </xsl:function>
    ...
    <xsl:template
        ...
        mode="determineTopicType" priority="10">
        ...
        <xsl:variable name="fallback" as="xs:string">
            <xsl:next-match/>
        </xsl:variable>
        <xsl:value-of
            select="
                if ($fallback = 'topicSimple') then
                    'topicFrontMatter'
                else
                    $fallback"
        />
