...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema"
    ...
    exclude-result-prefixes="opentopic-index opentopic opentopic-i18n opentopic-func dita-ot xs ot-placeholder" version="2.0">
    ...
    <xsl:variable name="productReleaseDate" select="(/*/opentopic:map//*[contains(@class, ' topic/vrm')]/@release)[1]"/>
  ...
  <xsl:variable name="pubDate" select="(/*/opentopic:map//*[contains(@class,' topic/revised ')]/@golive)[1]"/> -->
    ...
    <xsl:template match="/" mode="dita-ot:title-metadata" as="xs:string?">
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:value-of>
                    <xsl:apply-templates select="descendant::*[contains(@class, ' topic/topic ')][1]/*[contains(@class, ' topic/title ')]"
                        mode="dita-ot:text-only"/>
                </xsl:value-of>
            </xsl:otherwise>
        </xsl:choose>
    ...
    <xsl:template match="/" mode="dita-ot:author-metadata" as="xs:string?">
        <xsl:variable name="authorinformation"
            select="$map/*[contains(@class, ' bookmap/bookmeta ')]/*[contains(@class, ' xnal-d/authorinformation ')]" as="element()*"/>
        <xsl:choose>
            ...
            <xsl:when test="exists($authorinformation/descendant::*[contains(@class, ' xnal-d/organizationname ')])">
                <xsl:value-of>
                    <xsl:apply-templates select="$authorinformation/descendant::*[contains(@class, ' xnal-d/organizationname ')]"
                        mode="dita-ot:text-only"/>
                </xsl:value-of>
            </xsl:when>
    ...
    <xsl:template match="/" mode="dita-ot:keywords-metadata" as="xs:string*">
        <xsl:variable name="keywords" select="$map/*[contains(@class, ' bookmap/bookmeta ')]//*[contains(@class, 'topic/keyword ')]"
            as="element()*"/>
    ...
    <xsl:template match="/" mode="dita-ot:subject-metadata" as="xs:string?">
        <xsl:choose>
            ...
            <xsl:when test="exists($map/*[contains(@class, ' bookmap/bookmeta ')]/*[contains(@class, ' map/shortdesc ')])">
                <xsl:apply-templates
                    select="$map/*[contains(@class, ' bookmap/bookmeta ')]/*[contains(@class, ' map/shortdesc ')][not(./keyword)]"
                    mode="dita-ot:text-only"/>
