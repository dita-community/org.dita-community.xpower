<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:opentopic-func="http://www.idiominc.com/opentopic/exsl/function"
    xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:xs="http://www.w3.org/2001/XMLSchema"
    xmlns:dita2xslfo="http://dita-ot.sourceforge.net/ns/200910/dita2xslfo" xmlns:dita-ot="http://dita-ot.sourceforge.net/ns/201007/dita-ot"
                ...
                <number>
                    ...
                    <xsl:choose>
                        ...
                        <xsl:otherwise>
                            ...
                            -->
                            <!--HSX2 changed with DITA-OT 2.3.1  count(key replaced by apply-templates
                            <xsl:value-of select="count(key('enumerableByClass', 'topic/table')[. &lt;&lt; current()])"/>
                            <xsl:value-of select="count(key('enumerableByClass', 'topic/table')[. &lt;&lt; current()])"/>
                            -->
                            <xsl:apply-templates select="ancestor-or-self::*[contains(@class, ' topic/table ')][1]" mode="table.title-number"/>
                            <!--
                            -->
                        </xsl:otherwise>
                    </xsl:choose>
  ...
  <xsl:template match="*[contains(@class, ' topic/table ')]/*[contains(@class, ' topic/title ')]"
                        mode="table.title-number" priority="4">
      <xsl:value-of select="count(key('enumerableByClass', 'topic/table')[. &lt;&lt; current()])"/>
  </xsl:template>

  <xsl:template match="*[not(contains(@class, ' topic/table '))]" mode="table.title-number" priority="-1">
            ...
            <xsl:choose>
                <xsl:when test="../../title and contains($TitlePosition, 'table_titleRepeat')">
                    <fo:table-row>
                        ...
                        -->
                        <fo:table-cell background-color="white" border-top-color="white" border-top-style="solid" border-end-color="white"
                            border-end-style="solid" border-start-color="white" border-start-style="solid">
    ...
    <xsl:template match="*[contains(@class, ' topic/thead ')]/*[contains(@class, ' topic/row ')]/*[contains(@class, ' topic/entry ')]">
        <fo:table-cell xsl:use-attribute-sets="thead.row.entry">
            ...
            <xsl:variable name="inlProg">
                 <xsl:call-template name="getMaxEntry"/>
            </xsl:variable>
            <xsl:choose>
                ...
                <xsl:when test="@rotate">
                    <fo:block-container reference-orientation="{@rotate}" inline-progression-dimension="{$inlProg}">
                ...
                <xsl:when
                    ...
                    <fo:block-container reference-orientation="90" inline-progression-dimension="{$inlProg}">
                        <fo:block xsl:use-attribute-sets="thead.row.entry__content">
                            <xsl:apply-templates select="." mode="ancestor-start-flag"/>
                            <xsl:call-template name="processEntryContent"/>
                        </fo:block>
    ...
    <xsl:template name="getMaxEntry">
        ...
        <xsl:choose>
            <xsl:when test="$maxLength &gt; 24">
                <xsl:value-of select="'2in'"/>
            </xsl:when>
            <!--HSX I found the empirical factor 6.4pt to be a good match to 
                estimate the AVERAGE size of a character. We could go even harder
                for trying to involve the present font-size, but for now it is OK
            -->
            <xsl:otherwise>
                <xsl:value-of select="concat(string(6.4 * $maxLength),'pt')"/>                            
            </xsl:otherwise> 
        </xsl:choose>
    </xsl:template>
    <xsl:template match="*" mode="processTableEntry">
        ...
        <xsl:choose>
            <xsl:when test="@rotate">
                <fo:block-container reference-orientation="{@rotate}" inline-progression-dimension="$inProg">
