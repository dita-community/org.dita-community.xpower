...
-->
    ...
    <xsl:function name="test:image-height" as="xs:integer">
        ...
        <xsl:choose>
            <xsl:when test="ends-with(lower-case($gfxname), '.svg')">
                <xsl:value-of select="1"/>
            </xsl:when>
            <xsl:otherwise>
    ...
    <!--HSX I tried svg to get its size but this increased the processing time by a factor of 5 !
        Reading .svg is not possible (only in DITA-OT) but a .svg file can be presented as .xml
        which makes it work. Yet the factor 5 stays. Therefore I evaluate the sizes of SVG files
        with preprocessing  (svgExtractSize) to produce a svgsize.xml which only contain sthe
        size parameteres. That file can be read very fast and hence I get the size of the SVGs
    ...
    <xsl:template name="getImageWidth" as="xs:double">
        ...
        <xsl:variable name="pathname" select="concat($input.dir.url, $filename)" as="xs:string"/>
        <xsl:variable name="filepath" select="replace($pathname, 'file:[/]+', '')" as="xs:string"/>
        <xsl:variable name="sizename">
            <xsl:value-of select="concat($input.dir.url, substring-before($filename, '/'), '/svgsize.xml')"/>
        </xsl:variable>

        <xsl:choose>
            <xsl:when test="ends-with(lower-case($filename), '.svg')">
                <xsl:choose>
                    <xsl:when test="unparsed-text-available($sizename)">

                        <xsl:variable name="svgpath">
                            <xsl:call-template name="getPath">
                                <xsl:with-param name="url" select="$filepath"/>
                            </xsl:call-template>
                        </xsl:variable>

                        <xsl:variable name="svgext">
                            <xsl:call-template name="getExt">
                                <xsl:with-param name="url" select="$filepath"/>
                            </xsl:call-template>
                        </xsl:variable>

                        <xsl:variable name="svgname" select="substring-before(substring-after($filepath, $svgpath), $svgext)"/>
                        <xsl:variable name="svgfile" select="document($sizename)" as="node()"/>
                        <xsl:variable name="svg-width">
                            <xsl:apply-templates select="$svgfile/*" mode="getWidth">
                                <xsl:with-param name="svgname" select="$svgname"/>
                            </xsl:apply-templates>
                        </xsl:variable>
                        <xsl:choose>
                            <xsl:when test="string-length(normalize-space($svg-width))">
                                <xsl:call-template name="convertUnitx">
                                    <xsl:with-param name="numUnit" select="$svg-width"/>
                                    <xsl:with-param name="inUnitsOf" select="$unit"/>
                                </xsl:call-template>
                            </xsl:when>
                            <!--HSX no entry found for the file, prompt to run svgExtractSize.xsl -->
                            <xsl:otherwise>
                                <xsl:value-of select="99999"/>                                
                            </xsl:otherwise>
                        </xsl:choose>
                    </xsl:when>
                    <!--HSX svgsize.xml not available .... report ! -->
                    <xsl:otherwise>
                        <xsl:value-of select="99999"/>
                    </xsl:otherwise>
                </xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:variable name="pxwidth" as="xs:integer">
                    <xsl:value-of select="test:image-width($filepath)"/>
                </xsl:variable>
    ...
    <xsl:template name="getImageHeight" as="xs:double">
        ...
        <xsl:choose>
            <xsl:when test="ends-with(lower-case($filename), '.svg')">
                <xsl:choose>
                    <xsl:when test="unparsed-text-available($sizename)">
                        ...
                        <xsl:variable name="svg-height">
                            <xsl:apply-templates select="$svgfile/*" mode="getHeight">
                                <xsl:with-param name="svgname" select="$svgname"/>
                            </xsl:apply-templates>
                        </xsl:variable>
                        <xsl:choose>
                            <xsl:when test="string-length(normalize-space($svg-height))">
                                <xsl:call-template name="convertUnitx">
                                    <xsl:with-param name="numUnit" select="$svg-height"/>
                                    <xsl:with-param name="inUnitsOf" select="$unit"/>
                                </xsl:call-template>
                            </xsl:when>
                            <!--HSX no entry found for the file, prompt to run svgExtractSize.xsl -->
            ...
            <xsl:otherwise>
                <xsl:variable name="pxheight" as="xs:integer">
                    <xsl:value-of select="test:image-height($filepath)"/>
                </xsl:variable>
    ...
    <xsl:template match="*" mode="getWidth">
        <xsl:param name="svgname"/>
        <xsl:value-of select="concat(key('svgs', $svgname)/width, key('svgs', $svgname)/width-unit)"/>
    </xsl:template>

    <xsl:template match="*" mode="getHeight">
        <xsl:param name="svgname"/>
        <xsl:value-of select="concat(key('svgs', $svgname)/height, key('svgs', $svgname)/height-unit)"/>
    </xsl:template>
