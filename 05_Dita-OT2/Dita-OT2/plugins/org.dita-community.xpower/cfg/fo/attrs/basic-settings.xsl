...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  values are: chkproc todo gls prcattrs replid chghref chref chg42 copyel 
              oxycmt chklinks chkSubtopic imgscale 
              chkexpanse = check expanse factor for svg images
...
<!--HSC ... changes default page format 8,27 x 11,69 (210x297mm) is DIN A4 -->
    ...
    <xsl:template name="getPagewideMM">
        <xsl:param name="isLandscape" select="false()" as="xs:boolean"/>
        <xsl:variable name="pgwidthMM">
            <xsl:choose>
                <xsl:when test="$isLandscape">
                    <xsl:call-template name="convertUnits">
                        <xsl:with-param name="numUnit" select="$page-height"/>
        ...
        <xsl:value-of select="$pgwidthMM - $mrgleftMM - $mrgrightMM"/>
    </xsl:template>

    <xsl:variable name="pagewide">
        <xsl:variable name="rawwide">
            <xsl:call-template name="getPagewideMM"/>                        
        </xsl:variable>
        <xsl:value-of select="concat($rawwide, 'mm')"/>
    </xsl:variable>
    ...
    <xsl:template name="getPageheightMM">
        <xsl:param name="isLandscape" select="false()" as="xs:boolean"/>
        <xsl:variable name="pgheightMM">
            <xsl:choose>
                <xsl:when test="$isLandscape">
                    <xsl:call-template name="convertUnits">
                        <xsl:with-param name="numUnit" select="$page-width"/>
        ...
        <xsl:variable name="mrgtopMM">
            <xsl:call-template name="convertUnits">
                <xsl:with-param name="numUnit" select="$page-margin-top"/>
                <xsl:with-param name="inUnitsOf" select="'mm'"/>
            </xsl:call-template>
        </xsl:variable>
        <xsl:variable name="mrgbottomMM">
            <xsl:call-template name="convertUnits">
                <xsl:with-param name="numUnit" select="$page-margin-bottom"/>
                <xsl:with-param name="inUnitsOf" select="'mm'"/>
            </xsl:call-template>
        </xsl:variable>
        <xsl:value-of select="$pgheightMM - $mrgtopMM - $mrgbottomMM"/>
    </xsl:template>
