...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="getLowerCaseLang" as="xs:string">
    ...
    <!--HSX ALSO CHANGE IN F:\Dita-OT2\plugins\org.dita.pdf2\xsl\common -->
    <xsl:template name="getVariable">
        <xsl:param name="id" as="xs:string"/>
        <xsl:param name="params" as="document-node()*"/>
        <!--HSX do not output warning until the DITA-OT is fixed 2.1.1 
    <xsl:call-template name="output-message">
      <xsl:with-param name="msgnum">066</xsl:with-param>
      <xsl:with-param name="msgsev">W</xsl:with-param>
      <xsl:with-param name="msgparams">%1=getVariable</xsl:with-param>
    </xsl:call-template>
    -->
