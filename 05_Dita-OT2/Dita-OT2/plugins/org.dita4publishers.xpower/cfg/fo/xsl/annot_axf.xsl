...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema"
    ...
    xmlns:ot-placeholder="http://suite-sol.com/namespaces/ot-placeholder" xmlns:my="my:my"
    xmlns:ext="http://exslt.org/common"
    exclude-result-prefixes="ot-placeholder opentopic opentopic-index opentopic-func dita2xslfo xs my ext axf"
    version="2.0">

    <!--HSX //////////////////////// VERY IMPORTANT ////////////////////
         Do not delete the statement <xsl:output indent="no"/>
    ...
    <xsl:template name="oxy_insert_start">
        ...
        <xsl:analyze-string select="normalize-space($tStart)"
            regex='author="(.*?)".?timestamp="([0-9]+)T([0-9]+)\+([0-9]+)".*'>
            <xsl:matching-substring>
                <xsl:element name="author">
                    <xsl:value-of select="regex-group(1)"/>
                </xsl:element>
                ...
                <fo:inline>
                    ...
                    <xsl:if test="$pCst = 1">
                        ...
                        <xsl:variable name="color"
                            select="concat('rgb(', $oxyStart/red, ',' , $oxyStart/green,',', $oxyStart/blue, ')')"/>
    ...
    <xsl:template name="putLink">
        ...
        <!--<xsl:variable name="npc" select="normalize-space(.)"/>    -->
        <xsl:variable name="rawText" select="."/>
        <xsl:choose>
            ...
            <xsl:when test="contains($ezRead, 'link')">
                <!--HSX
                ...
                <xsl:analyze-string select="$theText" regex="{'(.*?)(\[.*?\])'}">
                    <xsl:matching-substring>
                        <xsl:variable name="pretext" select="regex-group(1)"/>
                    ...
                    <!-- no link found in current text
                         Important to put out the raw text, elements like 'codeblock' require line breaks
                         which we have eliminated in the first analyze-string.
                         
                         As a consequence, a 'codeblock' with an ezRead link will have no line breaks.
                    -->
                    <xsl:non-matching-substring>
                        <xsl:value-of select="."/>
                    </xsl:non-matching-substring>
            ...
            <!-- no ezRead support - do not evaluate but simply print the text() -->
            <xsl:otherwise>
                <xsl:value-of select="."/>
            </xsl:otherwise>
        </xsl:choose>
