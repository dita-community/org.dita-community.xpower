...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="/" mode="index-postprocess">
    <fo:block xsl:use-attribute-sets="__index__label __chapter__frontmatter__name__container" id="{$id.index}">
      <xsl:call-template name="getVariable">
    ...
    <fo:block id="{generate-id(.)}" xsl:use-attribute-sets="index.term">
      ...
      <xsl:if test="not($no-page)">
        <xsl:if test="$idxs">
          ...
          <!--HSX To change the format of page numbers, 
              [DtPrt#17.11] puts the index-page-citation-list between fo:inline
              20150520: doesn't work yet for the color, but 'bold' was accepted even without fo:inline
          -->
