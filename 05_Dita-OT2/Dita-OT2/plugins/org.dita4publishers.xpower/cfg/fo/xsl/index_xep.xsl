...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:variable name="AHF">0</xsl:variable>
  ...
  <xsl:template match="/" mode="index-postprocess">
    ...
    <!--HSC [DtPrt#17.13.1] allows changing the column count,
        I didnt' want it differently -->
  ...
  <xsl:template match="opentopic-index:index.entry" mode="index-postprocess">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <!--HSC [DtPrt#17.12] allows control of indentation, we need to add here ..
        <fo:block xsl:use-attribute-sets="index-indents">
        -->
  ...
  <xsl:template match="*" mode="make-index-ref">
    ...
    <fo:block id="{generate-id(.)}" xsl:use-attribute-sets="index.term">
      ...
      <xsl:if test="not($no-page)">
        <!--HSC [DtPrt#17.13.3] allows adding chapter number
         yet I dont' want it - so it is not implemented 
       -->
        <xsl:if test="$idxs">
          <xsl:copy-of select="$index.separator"/>
          <!--HSC To change the format of page numbers, 
              [DtPrt#17.11] puts the index-page-citation-list between fo:inline
              the index_xep (this) already contains this, but doesn't associate
              a separate attribute set. This can be added here ...
          <fo:inline>
          -->
