...
<!--HSC
   ...
   I checked build.xml and found the gen-list (calling dost.jar) responsible for the
...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:key name="topic" match="dita-merge/*[contains(@class, ' topic/topic ')]"
        use="concat('#', @id)"/>
    <xsl:key name="topic" match="dita-merge/dita"
        use="concat('#', *[contains(@class, ' topic/topic ')][1]/@id)"/>
...
-->
    <!--HSX unittest for messages " copyel chghref replid  calls prcattrs chg42 chref       -->
    ...
    <xsl:template match="
            *[contains(@class, ' topic/topic ')]
            [not(contains(../@class, ' glossgroup/glossgroup '))]
            | dita-merge/dita">
    ...
    <xsl:template match="
        ...
        <xsl:variable name="idcount">
            ...
            <xsl:for-each select="key('topicref', $parentId)">
                <xsl:value-of
                    select="count(preceding::*[@href = current()/@href]
                                              [not(ancestor::*[contains(@class, ' map/reltable ')])])
                                               + count(ancestor::*[@href = current()/@href])"
                />
            </xsl:for-each>
        </xsl:variable>
        <xsl:copy>
                ...
                <xsl:otherwise>
                    ...
                    <xsl:if test="contains($unittest, 'chg42')">
                        <xsl:message>
                            ...
                            <xsl:value-of
                                select="
                                    ancestor::*[contains(name(), 'concept') or
                    ...
                    <xsl:value-of
                        select="concat('#', $topic-id, '/', $topic-id, '_Connect_42_', $element-id)"
                    />
                </xsl:otherwise>
            </xsl:choose>
        </xsl:attribute>
        ...
        <xsl:if test="contains($unittest, 'copyel')">
            <xsl:message>
                ...
                <xsl:value-of select="."/>
                    ...
                    <xsl:text>  &#xA;ParentName =</xsl:text>
                    <xsl:value-of select="../../name()"/>
                    <xsl:text>  &#xA;ParentClass =</xsl:text>
                    <xsl:value-of select="../../@class"/>
            </xsl:message>
        </xsl:if>
        <!--HSX bug fix ... a glossentry with parent=glossgroup shall inherit its id from the glossgroup/@id
            That would allow to reference it since the xrefs to the glossentry are not updated by dost.jar.
            
            We test whether the parent is glossgroup in order to allow the change only on glossentry.
            We are convervative testing for glossentry too, so we exclude a following glossbody
            If that shall be touched too, just remove the check for glossentry
        -->
        <xsl:choose>
            <xsl:when test="(name() ='id') and
                            contains(../@class, ' glossentry/glossentry ') and
                            contains(../../@class, 'glossgroup/glossgroup ')">
                <xsl:attribute name="id" select="concat(../../@id, '_Connect_42_', ../@oid)"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:copy-of select="."/>
            </xsl:otherwise>
    ...
    <xsl:template match="@id[not(parent::*[contains(@class, ' topic/topic ')])]">
        ...
        <xsl:if test="contains($unittest, 'chg42')">
            <xsl:message>
                <xsl:text>&#xA;Child:Test-Applying href:connect_42_topicid_elementid to href=[</xsl:text>
                <xsl:value-of select="."/>
                <xsl:text>] parent = </xsl:text>
                <xsl:value-of
                    select="
                ...
                <xsl:text>_Connect_42_</xsl:text>
                <xsl:value-of select="."/>
                <xsl:variable name="current-idx"
                    select="concat(ancestor::*[contains(@class, ' topic/topic ')][1]/@id, '|', .)"/>
                <xsl:if test="not(generate-id(.) = generate-id(key('duplicate-id', $current-idx)[1]))">
                    <xsl:text>_</xsl:text>
                    <xsl:value-of select="generate-id()"/>
                </xsl:if>
            </xsl:message>
