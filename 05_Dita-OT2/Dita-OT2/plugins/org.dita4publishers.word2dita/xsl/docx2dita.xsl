...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema"
    xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:local="urn:local-functions"
    ...
    xmlns:relpath="http://dita2indesign/functions/relpath" exclude-result-prefixes="xs rsiwp stylemap local relpath xsi"
    version="2.0">
    ...
    <xd:doc xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" scope="stylesheet">
        <xd:desc>
            ...
            <xd:p>This transform is intended to be the base for more specialized transforms that provide style-specific
                overrides. The input to this transform is the document.xml file within a DOCX package. </xd:p>
    ...
    <xsl:param name="mediaDirUri" select="relpath:newFile($outputDir, 'gfx')" as="xs:string"/>
    ...
    <xsl:param name="topicExtension" select="'.dita'" as="xs:string"/>
    <!-- Extension for generated topic files -->
    <xsl:param name="fileNamePrefix" select="''" as="xs:string"/>
    <!-- Prefix for genenerated file names -->
    <xsl:param name="chartsAsTables" select="'false'" as="xs:string"/>
    <!-- When true, capture Word charts as tables with the chart data -->
    ...
    <!-- Prefix to add to image filenames when constructing image references
       in the result XML.
    -->
    <xsl:param name="imageFilenamePrefix" as="xs:string?" select="$fileNamePrefix"/>
    ...
    <xsl:variable name="warnOnUnstyledParasBoolean" select="matches($warnOnUnstyledParas, 'yes|true|1', 'i')"/>

    <!-- When true, use topic titles as navigation titles in generated
       topicrefs.
    -->
    <xsl:param name="generateNavtitles" as="xs:string" select="'true'"/>
    <xsl:variable name="generateNavtitlesBoolean" select="matches($generateNavtitles, 'yes|true|1', 'i')"/>

    <xsl:param name="debug" select="'false'" as="xs:string"/>
    <xsl:variable name="debugBoolean" as="xs:boolean" select="matches($debug, 'true|yes|1|on', 'i')"/>
    ...
    <xsl:variable name="rootTopicUrl" as="xs:string?" select="concat($finalRootTopicName, $topicExtension)"/>
    ...
    <xsl:output name="indented" method="xml" indent="yes"/>
    <xsl:include href="office-open-utils.xsl"/>
    <xsl:include href="wordml2simple.xsl"/>
    <xsl:include href="spreadsheetml2simple.xsl"/>
    <xsl:include href="wordml2simpleLevelFixup.xsl"/>
    <xsl:include href="wordml2simpleMathTypeFixup.xsl"/>
    <xsl:include href="wordml2simpleAddLevels.xsl"/>
    <xsl:include href="simple2dita.xsl"/>
    <xsl:include href="modeMapUrl.xsl"/>
    <xsl:include href="modeTopicUrl.xsl"/>
    <xsl:include href="resultdocs-xref-fixup.xsl"/>


    <xsl:template match="/" priority="10">
        <xsl:apply-templates select="." mode="report-parameters"/>
        <xsl:variable name="doDebug" as="xs:boolean" select="$debugBoolean"/>
        <xsl:variable name="stylesDoc" as="document-node()" select="document('styles.xml', .)"/>

        <xsl:message> + [INFO] ====================================</xsl:message>
        <xsl:message> + [INFO] Generating initial simple WP doc....</xsl:message>
        <xsl:message> + [INFO] ====================================</xsl:message>
        <xsl:variable name="simpleWpDocBase" as="element()">
            <xsl:call-template name="processDocumentXml">
                <xsl:with-param name="doDebug" as="xs:boolean" tunnel="yes" select="$doDebug"/>
                <xsl:with-param name="stylesDoc" as="document-node()" tunnel="yes" select="$stylesDoc"/>
            </xsl:call-template>
        </xsl:variable>
        ...
        <xsl:if test="$doSaveIntermediateDocs">
            <xsl:result-document format="indented" href="{$tempDoc}">
                <xsl:message> + [DEBUG] Intermediate simple WP doc saved as <xsl:sequence select="$tempDoc"
                    /></xsl:message>
        ...
        <xsl:if test="$doSaveIntermediateDocs">
            <xsl:variable name="tempDocLevelFixup" select="relpath:newFile($outputDir, 'simpleWpDocLevelFixup.xml')"
                as="xs:string"/>
            <xsl:result-document format="indented" href="{$tempDocLevelFixup}">
        ...
        <xsl:variable name="simpleWpDocMathTypeFixupResult" as="document-node()">
            <xsl:choose>
        ...
        <xsl:if test="$doSaveIntermediateDocs">
            <xsl:variable name="tempDocMathTypeFixup"
                select="relpath:newFile($outputDir, 'simpleWpDocMathTypeFixup.xml')" as="xs:string"/>
            <xsl:result-document format="indented" href="{$tempDocMathTypeFixup}">
                ...
                <xsl:sequence select="$simpleWpDocMathTypeFixupResult"/>
            </xsl:result-document>
        </xsl:if>
        ...
        <xsl:variable name="simpleWpDoc" as="document-node()">
            <xsl:document>
        ...
        <xsl:if test="$doSaveIntermediateDocs">
            <xsl:variable name="tempDocFixup" select="relpath:newFile($outputDir, 'simpleWpDocFixup.xml')"
                as="xs:string"/>
            <xsl:result-document format="indented" href="{$tempDocFixup}">
                <xsl:message> + [DEBUG] Fixed-up simple WP doc saved as <xsl:sequence select="$tempDocFixup"
                    /></xsl:message>
        ...
        <xsl:apply-templates select="$simpleWpDoc" mode="simpleWp-addLevels2">
            <xsl:with-param name="doDebug" as="xs:boolean" tunnel="yes" select="$doDebug"/>
        </xsl:apply-templates>

        <xsl:if test="$doSaveIntermediateDocs">
            <xsl:variable name="tempDocFixup" select="relpath:newFile($outputDir, 'simpleWpWithLevels.xml')"
                as="xs:string"/>
            <xsl:result-document format="indented" href="{$tempDocFixup}">
                <xsl:message> + [DEBUG] Simple WP doc with levels added saved as <xsl:sequence select="$tempDocFixup"
                    /></xsl:message>
                <xsl:sequence select="$simpleWithLevels"/>
            </xsl:result-document>
        </xsl:if>
        ...
        <xsl:apply-templates select="$simpleWithLevels/*">
            <xsl:with-param name="doDebug" as="xs:boolean" tunnel="yes" select="$doDebug"/>
    ...
    <xsl:template name="report-parameters" match="*" mode="report-parameters">
        <xsl:message> ========================================== DOCX 2 DITA Version: ^version^ - build ^buildnumber^ at
            ^timestamp^ Parameters: + styleMapUri = "<xsl:sequence select="$styleMapUri"/>" + mediaDirUri =
                "<xsl:sequence select="$mediaDirUri"/>" + rootMapName = "<xsl:sequence select="$rootMapName"/>" +
            rootTopicName = "<xsl:sequence select="$rootTopicName"/>" (<xsl:value-of select="$finalRootTopicName"/>) +
            submapNamePrefix= "<xsl:sequence select="$submapNamePrefix"/>" + rootMapUrl = "<xsl:sequence
                select="$rootMapUrl"/>" + rootTopicUrl = "<xsl:sequence select="$rootTopicUrl"/>" + topicExtension =
                "<xsl:sequence select="$topicExtension"/>" + fileNamePrefix = "<xsl:sequence select="$fileNamePrefix"/>"
            + filterBr = "<xsl:sequence select="$filterBr"/>" + filterTabs = "<xsl:sequence select="$filterTabs"/>" +
            language = "<xsl:sequence select="$language"/>" + outputDir = "<xsl:sequence select="$outputDir"/>" + debug
            = "<xsl:sequence select="$debug"/>" + includeWordBackPointers= "<xsl:sequence
                select="$includeWordBackPointersBoolean"/>" + chartsAsTables = "<xsl:sequence
                select="$chartsAsTablesBoolean"/>" + saveIntermediateDocs = "<xsl:sequence
                select="$saveIntermediateDocs"/>" Global Variables: + platform = "<xsl:sequence select="$platform"/>" +
            debugBoolean = "<xsl:sequence select="$debugBoolean"/>" + doSaveIntermediateDocs = "<xsl:sequence
                select="$doSaveIntermediateDocs"/>" ========================================== </xsl:message>
    </xsl:template>
