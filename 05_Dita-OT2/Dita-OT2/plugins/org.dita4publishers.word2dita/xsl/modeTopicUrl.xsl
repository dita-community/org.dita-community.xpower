...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="rsiwp:topic" mode="topic-url">   
    ...
    <xsl:variable name="result" select="concat('chapters/', $topicName, $topicExtension)"/>
    <xsl:if test="$doDebug">
  ...
  <xsl:template match="rsiwp:*" mode="topic-url">
    ...
    <xsl:sequence select="concat('chapters/topic_', $topicTitleFragment, '_', generate-id(.),format-time(current-time(),'[h][m][s]'), '.dita')"/>
  </xsl:template>
