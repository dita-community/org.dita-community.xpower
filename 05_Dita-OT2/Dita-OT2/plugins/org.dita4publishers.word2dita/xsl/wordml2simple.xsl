...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema"
    xmlns:mv="urn:schemas-microsoft-com:mac:vml" xmlns:mo="http://schemas.microsoft.com/office/mac/office/2008/main"
    ...
    xmlns:c="http://schemas.openxmlformats.org/drawingml/2006/chart" xmlns:local="urn:local-functions"
    xmlns:saxon="http://saxon.sf.net/" xmlns:rsiwp="http://reallysi.com/namespaces/generic-wordprocessing-xml"
    ...
    exclude-result-prefixes="a c pic xs mv mo ve o r m v w10 w wne wp local relpath saxon" version="2.0">
 ...
 -->
    ...
    <xd:doc xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl">
        <xd:desc>
            <xd:p>The absolute URL of the directory that contains the media objects extracted from the DOCX package. The
                presumption is that some process has copied the media/ directory from the DOCX package to an appropriate
                location relative to where the DITA XML will be generated.</xd:p>
        </xd:desc>
    </xd:doc>

    <xsl:key name="formats" match="stylemap:output" use="@name"/>
    <xsl:key name="styleMapsById" match="stylemap:style | stylemap:paragraphStyle | stylemap:characterStyle"
        use="@styleId"/>
    <xsl:key name="styleMapsByName" match="stylemap:style | stylemap:paragraphStyle | stylemap:characterStyle"
        use="lower-case(@styleName)"/>
    <xsl:key name="stylesById" match="w:style" use="@w:styleId"/>

    <xsl:variable name="styleMapDoc" as="document-node()" select="document($styleMapUri)"/>

    <xsl:variable name="imageRelType"
        select="'http://schemas.openxmlformats.org/officeDocument/2006/relationships/image'" as="xs:string"/>

    <xsl:variable name="font2UnicodeMapsCollectionUri" as="xs:string"
        select="
            concat('font2unicodeMaps', '?',
            'recurse=yes;',
            'select=*.xml'
            )"/>
...
-->
    ...
    <xsl:template match="/" name="processDocumentXml">
        ...
        <xsl:message> + [INFO] wordml2simple: Processing DOCX document.xml file to generate intermediate simpleML
            XML...</xsl:message>
        <xsl:message> + [INFO] styleMap=<xsl:sequence select="document-uri($styleMapDoc)"/></xsl:message>
        <xsl:message> + [INFO] Found <xsl:value-of select="count($font2UnicodeMaps)"/> font-to-Unicode
            maps</xsl:message>
        ...
        <xsl:variable name="relsDoc" as="document-node()?" select="document('../word/_rels/document.xml.rels', .)"/>

        <document sourceDoc="{document-uri(.)}">
            <xsl:apply-templates>
    ...
    <xsl:template match="w:p">
        ...
        <xsl:variable name="specifiedStyleId" as="xs:string" select="string(./w:pPr/w:pStyle/@w:val)"/>
        ...
        <xsl:variable name="styleName" as="xs:string" select="local:lookupStyleName(., $stylesDoc, $styleId)"/>
        <xsl:if test="$doDebug">
        ...
        <xsl:variable name="styleMapById" as="element()?" select="key('styleMapsById', $styleId, $styleMapDoc)[1]"/>
        ...
        <xsl:variable name="styleData" as="element()">
          ...
          filtered out in later processing phases. -->
                    <stylemap:paragraphStyle styleId="copy" structureType="block" tagName="p" topicZone="body"/>
                </xsl:when>
            ...
            -->
                    <stylemap:paragraphStyle styleId="MTDisplayEquation" structureType="block" tagName="equation-block"
                        topicZone="body"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:choose>
                        <xsl:when test="$specifiedStyleId = ''">
                            <xsl:variable name="contentString" as="xs:string"
                                select="
                                    if (string-length(normalize-space(.)) > 60)
                                    then
                                        concat(substring(normalize-space(.), 1, 60), '...')
                                    else
                                        normalize-space(.)
                                    "/>
                            <xsl:if test="$warnOnUnstyledParasBoolean">
                                <xsl:message> - [WARNING: Unstyled non-empty paragraph with content "<xsl:sequence
                                        select="$contentString"/>"</xsl:message>
                            </xsl:if>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:message> - [WARNING: No style mapping for paragraph with style "<xsl:sequence
                                    select="$styleName"/>" [<xsl:sequence select="$styleId"/>]</xsl:message>
                        </xsl:otherwise>
                    </xsl:choose>
                    <stylemap:paragraphStyle styleId="copy" structureType="block" tagName="p" topicZone="body"/>
                </xsl:otherwise>
        ...
        <xsl:if test="$doDebug">
            <xsl:message> + [DEBUG] match on w:p: structureType = "<xsl:sequence
                    select="string($styleData/@structureType)"/>"</xsl:message>
        </xsl:if>
        <xsl:choose>
            <xsl:when test="string($styleData/@structureType) = 'skip'">
                <xsl:if test="$doDebug">
                    <xsl:message> + [DEBUG] skipping paragraph with @structureType "<xsl:value-of
                            select="$styleData/@structureType"/>"</xsl:message>
                </xsl:if>
            </xsl:when>
            <!-- Skip it -->
            <xsl:when test=".//w:drawing//c:chart">
                <xsl:choose>
                    <xsl:when test="$chartsAsTablesBoolean">
                        ...
                        <!-- Put chart tables at same level as paragraphs -->
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:message> + [INFO] chartsAsTables is false, ignoring <xsl:value-of
                                select="string-join(.//w:drawing/*/wp:docPr/@name, ', ')"/></xsl:message>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:when>
            <xsl:otherwise>
                <xsl:if test="$doDebug">
                    <xsl:message> + [DEBUG] match on w:p: Paragraph not skipped, calling handlePara. p=<xsl:sequence
                            select="substring(string(./w:r[1]), 0, 40)"/></xsl:message>
                </xsl:if>
    ...
    <xsl:template name="handlePara">
        ...
        <xsl:if test="$styleData/self::stylemap:characterStyle">
            <xsl:message> - [WARN] Paragraph style "<xsl:value-of select="$styleId"/>" is a paragraph style but is
                mapped using a <xsl:value-of select="local-name($styleData)"/> element.</xsl:message>
        </xsl:if>
        ...
        -->
            ...
            <xsl:if test="$doDebug">
                <xsl:message> + [DEBUG] handlePara: p="<xsl:sequence select="substring(normalize-space(.), 1, 40)"
                    />"</xsl:message>
            </xsl:if>
        ...
        -->
            <xsl:for-each-group select="*" group-adjacent="name(.)">
                <xsl:if test="$doDebug">
                    <xsl:message> + [DEBUG] handlePara: current-group()[1]=<xsl:sequence select="current-group()[1]"
                        /></xsl:message>
                </xsl:if>
                <xsl:choose>
                    ...
                    <xsl:when test="current-group()[1][self::w:smartTag]">
                        <xsl:if test="$doDebug">
                            <xsl:message> + [DEBUG] handlePara: *** got a w:smartTag. current-group=<xsl:sequence
                                    select="current-group()"/></xsl:message>
                        </xsl:if>
    ...
    <xsl:function name="local:getTagnameFromNestedProperties">
        ...
        <xsl:variable name="tagName" as="xs:string?">
            <xsl:choose>
                ...
                <xsl:when test="$styleData/stylemap:topicrefProperties and count($styleData/*) = 1">
                    <xsl:sequence
                        select="
                            if ($styleData/stylemap:topicrefProperties/@tagName)
                            then
                ...
                <xsl:when test="$styleData/stylemap:mapProperties and count($styleData/*) = 1">
                    <xsl:sequence
                        select="
                            if ($styleData/stylemap:mapProperties/@tagName)
                            then
                                $styleData/stylemap:mapProperties/@tagName
                            else
                                'title'
                            "
                    />
                ...
                <xsl:otherwise>
                    <xsl:sequence
                        select="
                            ($styleData/stylemap:topicProperties/@tagName,
                            $styleData/stylemap:topicrefProperties/@tagName,
        ...
        <xsl:if test="$tagName = ''">
            <xsl:message> - [ERROR] No @tagName attribute for style "<xsl:value-of select="$styleData/@styleName"
                />".</xsl:message>
        </xsl:if>
        <xsl:if test="$tagName != ''">
    ...
    <xsl:template name="handleRunSequence">
        ...
        <xsl:variable name="styleId"
            select="
                if ($runSequence[1]) then
                    local:getRunStyleId($runSequence[1])
                else
                    ''"
            as="xs:string"/>
        <xsl:if test="$doDebug">
        ...
        <xsl:choose>
            <xsl:when test="$styleId = ''">
                <xsl:if test="$doDebug">
                    <xsl:message> + [DEBUG] handleRunSequence: No style ID, applying templates to run
                        sequence...</xsl:message>
                </xsl:if>
          ...
          -->
                <xsl:choose>
                    <xsl:when test="$styleName = 'MTConvertedEquation'">
                        <stylemap:characterStyle styleId="MTConvertedEquation" structureType="ph" tagName="mathml"/>

                    </xsl:when>
                    <xsl:when test="not($runStyleMap)">
                        <xsl:message> - [WARNING: No style mapping for character run with style "<xsl:sequence
                                select="$styleName"/>" [<xsl:sequence select="$styleId"/>]</xsl:message>
                    </xsl:when>
                </xsl:choose>
                <xsl:variable name="runTagName" as="xs:string"
                    select="
                        if ($runSequence[1][self::w:hyperlink])
                        then
                            'hyperlink'
                        else
                            'run'
                        "/>
                <xsl:if test="$doDebug">
                    <xsl:message> + [DEBUG] handleRunSequence: runTagName="<xsl:value-of select="$runTagName"/>",
                        generating result element...</xsl:message>
                </xsl:if>
    ...
    <xsl:template match="w:footnoteReference">
        ...
        <xsl:variable name="footnotesDoc" as="document-node()?" select="document('footnotes.xml', .)"/>
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:variable name="targetId" as="xs:string" select="@w:id"/>
                <fn>
                    <xsl:apply-templates select="$footnotesDoc/*/w:footnote[@w:id = $targetId]"/>
                </fn>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template match="w:endnoteReference">
        <xsl:param name="doDebug" as="xs:boolean" tunnel="yes" select="false()"/>
        <xsl:variable name="endnotesDoc" as="document-node()?" select="document('endnotes.xml', .)"/>
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:variable name="targetId" as="xs:string" select="@w:id"/>
                <fn>
                    <xsl:apply-templates select="$endnotesDoc/*/w:endnote[@w:id = $targetId]"/>
                </fn>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template match="w:footnote | w:endnote">
        <xsl:param name="doDebug" as="xs:boolean" tunnel="yes" select="false()"/>
        <xsl:apply-templates/>
        <!-- FIXME: May need to be more selective here. -->
    </xsl:template>
    ...
    <xsl:template
        match="
            w:r[w:rPr/w:rFonts[w:rPr/w:rFonts/@w:ascii = ('Symbol',
            'Wingdings')]]"
        priority="10">
        <xsl:variable name="fontFace" as="xs:string?" select="w:rPr/w:rFonts/@w:ascii"/>
        <!--        <xsl:message> + [DEBUG] ==== w:r[w:rPr/w:rFonts]: fontFace="<xsl:sequence select="$fontFace"/>"</xsl:message>-->

        <!-- Treat the content as for w:sym -->
        <xsl:variable name="text" as="xs:string" select="string(w:t)"/>
        <xsl:for-each select="string-to-codepoints($text)">
            <xsl:variable name="codePoint" as="xs:string" select="local:int-to-hex(.)"/>
            <!--          <xsl:message> + [DEBUG] codePoint="<xsl:sequence select="$codePoint"/>"</xsl:message>-->
            <xsl:sequence
                select="
                    local:constructSymbolForCharcode(
                    $codePoint,
                    $fontFace)"
            />
        </xsl:for-each>
        <!--HSX respect the rest of the template -->
        <xsl:apply-templates
            select="
                *[not(w:rPr/w:rFonts[w:rPr/w:rFonts/@w:ascii = ('Symbol',
                'Wingdings')])]"/>
        <!--HSX end -->
    </xsl:template>

    <!--HSX trying to support subscript/superscript -->
    <xsl:template match="w:rPr" priority="4">
        <xsl:apply-templates/>
    </xsl:template>

    <!--HSX unhandled tags -->
    <xsl:template match="w:lang" priority="4"/>
    <xsl:template match="w:rPr/w:rFonts" priority="4"/>
    <xsl:template match="w:rPr/w:color" priority="4"/>
    <xsl:template match="w:rPr/w:sz" priority="4"/>
    <xsl:template match="w:rPr/w:szCs" priority="4"/>
    <xsl:template match="w:tblPr/w:tblLayout" priority="4"/>
    <xsl:template match="w:tblPr/w:tblpPr" priority="4"/>
    <xsl:template match="w:trPr/w:jc" priority="4"/>
    <xsl:template match="w:rPr/w:u" priority="4"/>
    <xsl:template match="w:rPr/w:b" priority="4"/>
    <xsl:template match="w:rPr/w:bCs" priority="4"/>
    <xsl:template match="w:tblPr/w:jc" priority="4"/>
    <xsl:template match="w:tblPr/w:tblCellMar" priority="4"/>
    
    <xsl:template match="w:r/w:t" priority="4" mode="subText">
        <xsl:value-of select="."/>
    </xsl:template>

    <xsl:template match="w:r[not(w:rPr/w:vertAlign)]/w:t" priority="4">
        <xsl:value-of select="."/>
    </xsl:template>

    <xsl:template match="w:r[w:rPr/w:vertAlign]/w:t" priority="4"/>

    <xsl:template match="w:vertAlign" priority="10">
        <xsl:choose>
            <xsl:when test="@w:val = 'subscript'">
                <sub>
                    <xsl:apply-templates select="../../w:t" mode="subText"/>
                </sub>
            </xsl:when>
    ...
    <xsl:template
        match="
            w:r[not(w:rPr/w:rFonts[w:rPr/w:rFonts/@w:ascii = ('Symbol',
            'Wingdings')])]">
        <xsl:apply-templates/>
    </xsl:template>
    <!--HSX end-->
    ...
    <xsl:template match="w:bookmarkStart">
        <xsl:param name="doDebug" as="xs:boolean" tunnel="yes" select="false()"/>
        <bookmarkStart name="{@w:name}" id="{@w:id}"/>
    </xsl:template>
    ...
    <xsl:template match="w:hyperlink">
        ...
        <xsl:variable name="styleMapById" as="element()?" select="key('styleMapsById', $runStyle, $styleMapDoc)[1]"/>
        <xsl:variable name="runStyleMap" as="element()?"
            select="
                ($styleMapByName,
                $styleMapById)[1]"/>
        <xsl:variable name="runStyleData" as="element()">
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <stylemap:characterStyle styleId="Hyperlink" structureType="xref" tagName="xref"/>

                </xsl:otherwise>
            </xsl:choose>

        </xsl:variable>
        <xsl:variable name="rel" as="element()?" select="key('relsById', @r:id, $relsDoc)"/>
      ...
      -->
        ...
        <hyperlink href="{$href}">
            ...
            <xsl:if test="count(w:r) > 1">
                <xsl:if test="$doDebug">
                    <xsl:message> + [DEBUG] hyperlink: More runs, calling handleRunSequence on grouped
                        runs...</xsl:message>
                </xsl:if>
                <xsl:for-each-group select="w:r[position() > 1]" group-adjacent="local:getRunStyleId(.)">
                    <xsl:if test="$doDebug">
                        <xsl:message> + [DEBUG] hyperlink: grouping key="<xsl:value-of select="current-grouping-key()"
                            />"</xsl:message>
                    </xsl:if>
                    <xsl:call-template name="handleRunSequence">
                        <xsl:with-param name="doDebug" as="xs:boolean" tunnel="yes" select="$doDebug"/>
    ...
    <!--HSX I put the condition up to handle vertical alignment sub/sup seperately-->
    <xsl:template match="w:t[not(preceding-sibling::w:rPr[1]/w:vertAlign)]">
        <xsl:param name="doDebug" as="xs:boolean" tunnel="yes" select="false()"/>
    ...
    <xsl:template match="w:tbl">
        <xsl:param name="doDebug" as="xs:boolean" tunnel="yes" select="false()"/>
        <xsl:variable name="styleData" as="element()">
            <stylemap:paragraphStyle styleId="table" structureType="block" tagName="table" topicZone="body"/>
        </xsl:variable>
                        ...
                        <xsl:variable name="widthValPoints" as="xs:string"
                            select="
                                ...
                                then
                                    ...
                                    concat(format-number((number(@w:w) div 20), '######.00'), 'pt')"/>
    ...
    <xsl:function name="local:calculateTableCellHorizontalAlignment" as="xs:string?">
      ...
      -->
        ...
        <xsl:variable name="jcValue" as="xs:string?" select="($tcElement/w:p/w:pPr/w:jc)[1]/@w:val"/>
        ...
        <xsl:variable name="result" as="xs:string?"
            select="
                ...
                then
                    ...
                    'justify',
                    'char',
                    'left')[index-of($wordJustificationValues, $jcValue)]
                else
                    ()"/>
    ...
    <xsl:function name="local:calculateTableCellVerticalAlignment" as="xs:string?">
      ...
      -->
        <xsl:param name="tcElement" as="element(w:tc)"/>
        <xsl:variable name="valignValue" as="xs:string?" select="$tcElement/w:tcPr/w:vAlign/@w:val"/>
        ...
        <xsl:variable name="result" as="xs:string?"
            select="
                ...
                then
                    ...
                    'middle',
                    'top',
                    'top')[index-of($wordJustificationValues, $valignValue)]
                else
                    ()"/>
        <xsl:sequence select="$result"/>
    </xsl:function>
            ...
            <xsl:choose>
                <xsl:when test="$tblGrid">
                    <xsl:variable name="widthValues"
                        select="
                            for $att in $tblGrid/w:gridCol/@w:w
                            return
                                number($att)"/>
                    <xsl:variable name="sum" select="sum($widthValues)"/>
                    <xsl:sequence select="string($sum div 20)"/>
                    <!-- Value in points -->
                </xsl:when>
                <xsl:otherwise>
                    <xsl:sequence select="'0'"/>
                    <!-- Width not calculatable -->
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        <xsl:sequence select="$result"/>
        ...
        <xsl:variable name="result" as="xs:string">
            <xsl:choose>
                <xsl:when test="$tblBorders">
                    <xsl:variable name="borderTop" as="xs:boolean" select="$tblBorders/w:top/@w:val != 'nil'"/>
                    <xsl:variable name="borderBottom" as="xs:boolean" select="$tblBorders/w:bottom/@w:val != 'nil'"/>
                    <xsl:variable name="borderLeft" as="xs:boolean" select="$tblBorders/w:left/@w:val != 'nil'"/>
                    <xsl:variable name="borderRight" as="xs:boolean" select="$tblBorders/w:right/@w:val != 'nil'"/>
                    <xsl:choose>
    ...
    <xsl:template match="w:tr">
        <xsl:param name="doDebug" as="xs:boolean" tunnel="yes" select="false()"/>
        <xsl:variable name="isHeaderRow" as="xs:boolean" select="boolean(w:trPr/w:tblHeader)"/>
    ...
    <xsl:template match="w:tc">
        ...
        <xsl:variable name="horizontalAlignment" as="xs:string?" select="local:calculateTableCellHorizontalAlignment(.)"/>
        <xsl:variable name="verticalAlignment" as="xs:string?" select="local:calculateTableCellVerticalAlignment(.)"/>
    ...
    <xsl:template match="w:cr | w:br">
        ...
        <xsl:if test="not($filterBrBoolean)">
            <xsl:if test="$doDebug">
                <xsl:message> + [DEBUG] <xsl:value-of select="name(.)"/>: Not filtered, generating &lt;break;&gt;
                    element.</xsl:message>
            </xsl:if>
            <break>
    ...
    <xsl:template match="w:drawing">
        ...
        <xsl:variable name="xExtentEmu" as="xs:double"
            select="
                if ($xExtentStr castable as xs:integer) then
                    number($xExtentStr)
                else
                    1800000.0"/>
        <xsl:variable name="yExtentEmu" as="xs:double"
            select="
                if ($yExtentStr castable as xs:integer) then
                    number($yExtentStr)
                else
                    1800000.0"/>
    ...
    <xsl:template match="w:sym">
        <xsl:param name="doDebug" as="xs:boolean" tunnel="yes" select="false()"/>
        <xsl:variable name="fontFace" as="xs:string?" select="@w:font"/>
        <xsl:choose>
            ...
            <xsl:otherwise>
                <rsiwp:symbol font="{$fontFace}">
                    <xsl:sequence select="string(@w:char)"/>
                </rsiwp:symbol>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:function name="local:constructSymbolForCharcode" as="node()*">
        ...
        <xsl:variable name="nonPrivateCharCode" as="xs:string"
            select="
                if (starts-with($charCode, 'F')) then
                    replace($charCode, 'F', '0')
                else
                    $charCode"/>
        ...
        <xsl:variable name="codePoint" as="xs:integer" select="local:hex-to-char($unicodeCodePoint)"/>
        <xsl:variable name="character" select="codepoints-to-string($codePoint)" as="xs:string"/>
        <rsiwp:symbol font="{$fontFace}">
    ...
    <xsl:template match="a:blip">
        ...
        <xsl:variable name="imageUri" as="xs:string" select="local:getImageReferenceUri($relsDoc, @r:embed, @r:link)"/>
        <xsl:if test="$doDebug">
            <xsl:message> + [DEBUG] a:blip: imageUri="<xsl:value-of select="$imageUri"/>"</xsl:message>
        </xsl:if>
        <image src="{$imageUri}" width="{$width}" height="{$height}"/>
    </xsl:template>
    ...
    <xsl:template match="v:imagedata">
	    ...
	    -->
        <xsl:variable name="imageUri" as="xs:string" select="local:getImageReferenceUri($relsDoc, @r:id, @r:link)"/>
        <image src="{$imageUri}"/>
    </xsl:template>
    ...
    <xsl:template match="c:chart">
        ...
        <xsl:variable name="chartDoc" as="document-node()" select="document($targetUri, root(.))"/>
        <xsl:choose>
            <xsl:when test="not($chartDoc)">
                <xsl:message> - [WARN] Failed to resolve reference to chart document "<xsl:sequence select="$targetUri"
                    />"</xsl:message>
            </xsl:when>
            <xsl:otherwise>
                ...
                <xsl:choose>
                    <xsl:when test="$chartDoc/*/c:externalData">
                        ...
                        <xsl:variable name="extDataId" as="xs:string" select="$chartDoc/*/c:externalData/@r:id"/>
                        <xsl:variable name="relsDocURI" as="xs:string"
                            select="concat('../word/charts/_rels/', $chartname, '.xml.rels')"/>
                        <xsl:variable name="chartsRelsDoc" as="document-node()?" select="document($relsDocURI, .)"/>
                        <xsl:variable name="rel" as="element()?" select="key('relsById', $extDataId, $chartsRelsDoc)"/>
                        <xsl:variable name="extDataRelativeUri" as="xs:string" select="$rel/@Target"/>
                        <xsl:variable name="extDataAbsolutePath" as="xs:string"
                            select="
                                ...
                                $extDataRelativeUri))"/>
                        <xsl:variable name="extDataURI" as="xs:string" select="concat('zip:', $extDataAbsolutePath)"/>
                        <xsl:variable name="spreadsheetDocURI" as="xs:string"
                            select="concat($extDataURI, '!', '/xl/worksheets/sheet1.xml')"/>
                        <xsl:variable name="spreadsheetDoc" as="document-node()?" select="document($spreadsheetDocURI)"/>
                        <xsl:choose>
                            <xsl:when test="$spreadsheetDoc">
                                <xsl:apply-templates select="$spreadsheetDoc" mode="spreadsheet-to-cals-table"/>
                            </xsl:when>
                            <xsl:otherwise>
                                <xsl:message> - [WARN] Failed to get spreadsheet document for chart element using URI
                                        "<xsl:sequence select="$spreadsheetDocURI"/>"</xsl:message>
                            </xsl:otherwise>
                        </xsl:choose>

                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:message> - [WARN] No c:externalData element for chart "<xsl:value-of select="$targetUri"
                            />"</xsl:message>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:otherwise>
    ...
    <xsl:template match="w:sdt">
        ...
        <xsl:variable name="alias" as="xs:string?" select="w:sdtPr/w:alias/@w:val"/>
        <xsl:variable name="tagname" as="xs:string?" select="w:sdtPr/w:tag/@w:val"/>
        <xsl:variable name="dataBinding" as="xs:string?" select="w:sdtPr/w:dataBinding/@w:xpath"/>
        <xsl:variable name="prefixMappings" as="xs:string?" select="w:sdtPr/w:dataBinding/@w:prefixMappings"/>
        <xsl:variable name="dataPartId" as="xs:string?" select="w:sdtPr/w:dataBinding/@w:storeItemID"/>
    ...
    <xsl:function name="local:getImageReferenceUri" as="xs:string">
        ...
        <xsl:param name="linkId" as="xs:string?"/>
        <!-- ID of any external link to the image. -->

        <xsl:variable name="rel" as="element()?" select="key('relsById', $relId, $relsDoc)"/>
        <xsl:variable name="linkRel"
            select="
                if ($linkId)
                then
                    key('relsById', $linkId, $relsDoc)
                else
                    ()"
            as="element()?"/>
        <xsl:variable name="target" as="xs:string"
            select="
                ...
                then
                    string($linkRel/@Target)
                else
                    string($rel/@Target)"/>
        <xsl:variable name="imageBasename" as="xs:string" select="relpath:getName($target)"/>
        <xsl:variable name="imageFilename" as="xs:string"
            select="
                if ($imageFilenamePrefix)
                then
                    concat($imageFilenamePrefix, $imageBasename)
                else
                    $imageBasename"/>

        <xsl:variable name="srcValue" as="xs:string" select="relpath:newFile($mediaDirUri, $imageFilename)"/>
        <xsl:choose>
            <xsl:when test="$rel and string($rel/@Type) = $imageRelType">
                <xsl:sequence select="$srcValue"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:message> - [WARNING] getImageReferenceUri(): Failed to find Relationship of type Image for
                    relationship ID "<xsl:sequence select="$relId"/>"</xsl:message>
                <xsl:sequence select="concat('unresolvedgraphic_id_', $relId)"/>
            </xsl:otherwise>
        </xsl:choose>
    ...
    <xsl:function name="local:hex-to-char" as="xs:integer">
        ...
        <xsl:sequence
            select="
                ...
                then
                    local:hex-digit-to-integer($in)
                else
                    16 * local:hex-to-char(substring($in, 1, string-length($in) - 1)) +
	...
	-->
        ...
        <xsl:choose>
            <xsl:when test="not($fontCharMap)">
                <xsl:message> - [WARN] getUnicodeForFont(): No font-to-character map found for font "<xsl:value-of
                        select="$fontName"/>"</xsl:message>
                <xsl:sequence select="'003F'"/>
            </xsl:when>
            
            <xsl:when test="empty($fontCharMap/codePointMapping[@origCodePoint = $fontCodePoint])">
                <xsl:sequence select="'003F'"/>
            </xsl:when>
            ...
            <xsl:otherwise>
                <xsl:variable name="unicodeCodePoint" as="xs:string">
                    ...
                    <!--HSX trouble here "empty sequence" ... fixing by ignorance -->
    ...
    <xsl:function name="local:hex-digit-to-integer" as="xs:integer">
        ...
        <xsl:sequence
            select="
                string-length(substring-before('0123456789ABCDEF',
                $char))"/>
    </xsl:function>
    ...
    <xsl:template match="w:*" priority="-0.5">
        <xsl:param name="doDebug" as="xs:boolean" tunnel="yes" select="false()"/>
        <xsl:message> - [WARNING] wordml2simple: Unhandled element <xsl:sequence select="name(..)"/>/<xsl:sequence
