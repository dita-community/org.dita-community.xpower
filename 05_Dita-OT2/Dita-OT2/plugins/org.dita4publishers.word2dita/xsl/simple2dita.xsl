...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
    ...
    <xsl:include href="postProcess.xsl"/>
...
<xsl:sequence select="$tempDoc"/> 
    ...
    <xsl:message> + [INFO] simple2dita: Phase 2: Generate the result map and topic documents...</xsl:message>
      ...
      <!--HSX included post processing -->
      <xsl:variable name="resultDocsFixedUp2" as="node()*">
          <xsl:apply-templates select="$resultDocsFixedUp" mode="docs-post">
              <xsl:with-param name="simpleWpDoc" as="document-node()" tunnel="yes"
                  select="$simpleWpDoc"/>                  
          </xsl:apply-templates>
      </xsl:variable>
      
      <!--HSC normal continuation -->
      <xsl:apply-templates select="$resultDocsFixedUp2" mode="generate-result-docs">
      <xsl:with-param name="simpleWpDoc" as="document-node()" tunnel="yes"
        select="$simpleWpDoc"
      />                  
    </xsl:apply-templates>
