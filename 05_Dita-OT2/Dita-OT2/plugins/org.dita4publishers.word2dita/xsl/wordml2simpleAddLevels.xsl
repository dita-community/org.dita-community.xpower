...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema"
    ...
    xmlns:v="urn:schemas-microsoft-csimpleWp-addLevelsom:vml" xmlns:w10="urn:schemas-microsoft-com:office:word"
    ...
    xmlns:mathml="http://www.w3.org/1998/Math/MathML" xmlns:local="urn:local-functions"
    ...
    exclude-result-prefixes="a pic xs mv mo ve o r m v w10 w wne wp local relpath saxon" version="2.0">
    ...
    <xsl:template mode="simpleWp-addLevels" match="/">
        ...
        <!-- <xsl:if test="$doSaveIntermediateDocs">
            <xsl:variable name="tempDocFixup"
                select="relpath:newFile($outputDir, 'simpleWpWithLevelsNoContainerTypes.xml')" as="xs:string"/>
    ...
    <xsl:template mode="simpleWp-addLevels2" match="/">
        <xsl:param name="doDebug" as="xs:boolean" tunnel="yes" select="false()"/>
        <!-- Construct the map, topicref, and topic hierarchy, but without container types. -->
    ...
    <xsl:template mode="simpleWp-addLevels" match="rsiwp:body">
      ...
      -->
        ...
        <xsl:if
            ...
            <xsl:message terminate="yes"> - [ERROR] The first paragraph in the Word document must be mapped to the root
                map or topic title. First para is style <xsl:sequence select="string($firstP/@style)"/>, mapped as
                    <xsl:sequence
                    select="
    ...
    <xsl:template name="groupMapsAndTopicsByLevel">
      ...
      -->
        ...
        <xsl:if test="$doDebug">
            <xsl:message> + [DEBUG] groupMapsAndTopicsByLevel: level=<xsl:value-of select="$level"/>,
                    content[1]=<xsl:value-of select="local:reportPara($content[1])"/></xsl:message>
        </xsl:if>
        ...
        -->
            <xsl:choose>
                <xsl:when test=".[not(local:isMapOrTopicStructure(.))]">
                    <xsl:if test="$doDebug">
                        <xsl:message> + [DEBUG] groupMapsAndTopicsByLevel: Group is not a map structure, emitting
                            it.</xsl:message>
                    </xsl:if>
                ...
                <xsl:otherwise>
                    <xsl:if test="$doDebug">
                        <xsl:message> + [DEBUG] groupMapsAndTopicsByLevel: Group is a map structure, applying
                            addLevels-map...</xsl:message>
                    </xsl:if>
                    <xsl:apply-templates select="current-group()[1]" mode="addLevels-map">
                        <xsl:with-param name="doDebug" as="xs:boolean" tunnel="yes" select="$doDebug"/>
                        <xsl:with-param name="rest" as="element()*" tunnel="yes"
    ...
    <xsl:template mode="addLevels-map" match="*[@generatesMap = 'true']">
        <xsl:param name="doDebug" as="xs:boolean" tunnel="yes" select="false()"/>
      ...
      -->
        ...
        <xsl:if test="$doDebug">
            <xsl:message> + [DEBUG] addLevels-map: map or mapTitle: <xsl:value-of select="local:reportPara(.)"
                /></xsl:message>
        </xsl:if>
        ...
        <xsl:if test="not($outputElem)">
            <xsl:message> - [ERROR] No &lt;output&gt; element for format "<xsl:value-of
                    select="stylemap:mapProperties/@format"/>"</xsl:message>
        </xsl:if>
    ...
    <xsl:template mode="addLevels-topicref"
        match="
            *[@structureType = ('topicHead',
            'topicGroup')]" priority="10">
        <xsl:param name="doDebug" as="xs:boolean" tunnel="yes" select="false()"/>
        <xsl:param name="rest" as="element()*" tunnel="yes"/>

        <xsl:variable name="level" as="xs:integer" select="@level"/>
        <xsl:if test="$doDebug">
            <xsl:message> + [DEBUG] addLevels-topicref: topicHead/Group: level="<xsl:value-of select="$level"
                />"</xsl:message>
        </xsl:if>

        <xsl:variable name="content" as="element()*" select="$rest"/>
        <xsl:if test="false() and $doDebug">
            <xsl:message> + [DEBUG] addLevels-topicref: topicHead/Group: content="<xsl:value-of
                    select="local:reportParas($content)"/><xsl:value-of select="$level"/>"</xsl:message>
        </xsl:if>
        ...
        <xsl:variable name="navtitleType" as="xs:string"
            select="
                if ((@navtitleType,
                stylemap:topicrefProperties/@navtitleType)[1])
                then
                    (@navtitleType,
                    stylemap:topicrefProperties/@navtitleType)[1]
                else
        ...
        <xsl:element name="rsiwp:{@structureType}">
            ...
            <xsl:if test="not(starts-with($navtitleType, '#'))">
                <xsl:apply-templates select="." mode="addLevels-navtitle">
                    <xsl:with-param name="doDebug" as="xs:boolean" tunnel="yes" select="$doDebug"/>
                    <xsl:with-param name="tagName" as="xs:string?" select="$navtitleType"/>
                </xsl:apply-templates>
            </xsl:if>
            <xsl:call-template name="groupMapsAndTopicsByLevel">
                <xsl:with-param name="doDebug" as="xs:boolean" tunnel="yes" select="$doDebug"/>
    ...
    <xsl:template mode="addLevels-topicref" match="*" priority="-1">
        <xsl:param name="doDebug" as="xs:boolean" tunnel="yes" select="false()"/>
        <xsl:if test="$doDebug">
            <xsl:message> + [DEBUG] addLevels-topicref: catch-all: <xsl:value-of select="local:reportPara(.)"
                /></xsl:message>
        </xsl:if>
    ...
    <xsl:template mode="addLevels-topicref" match="*[@generatesTopicref = 'true']">
        <xsl:param name="doDebug" as="xs:boolean" tunnel="yes" select="false()"/>
    ...
    <xsl:template mode="addLevels-topic" priority="10" match="*[@generatesTopic = 'true']">
        <xsl:param name="doDebug" as="xs:boolean" tunnel="yes" select="false()"/>
        <xsl:if test="$doDebug">
            <xsl:message> + [DEBUG] addLevels-topicref: <xsl:value-of select="local:reportPara(.)"/></xsl:message>
        </xsl:if>
        ...
        -->
            ...
            <xsl:sequence select="stylemap:topicProperties/@*"/>
            <xsl:sequence select="."/>
    ...
    <xsl:template mode="addLevels-topic" match="*" priority="-1">
        ...
        <xsl:if test="$doDebug">
            <xsl:message> + [DEBUG] addLevels-topic: Catch-all: <xsl:value-of select="local:reportPara(.)"
                /></xsl:message>
        </xsl:if>
        <xsl:apply-templates mode="addLevels-handleChildren" select=".">
            <xsl:with-param name="doDebug" as="xs:boolean" tunnel="yes" select="$doDebug"/>
    ...
    <xsl:template mode="addLevels-handleChildren" match="*">
        ...
        <xsl:if test="$doDebug">
            <xsl:message> + [DEBUG] addLevels-handleChildren: <xsl:value-of select="local:reportPara(.)"/>,
    ...
    <xsl:template mode="handleContainerTypes"
        ...
        <xsl:if test="$doDebug">
            <xsl:message> + [DEBUG] handleContainerTypes: <xsl:value-of select="concat(name(..), '/', name(.))"
                /></xsl:message>
        </xsl:if>
        ...
        <xsl:copy>
            ...
            <xsl:for-each-group select="*" group-adjacent="concat('x', @containerType)">
                <xsl:if test="$doDebug">
                    <xsl:message> + [DEBUG] handleContainerTypes: Group <xsl:value-of select="position()"
                        /></xsl:message>
                </xsl:if>
                <xsl:choose>
                    ...
                    <xsl:when test="current-grouping-key() != 'x'">
                        <xsl:if test="$doDebug">
                            <xsl:message> + [DEBUG] handleContainerTypes: containerType="<xsl:value-of
                                    select="@containerType"/>"</xsl:message>
                        </xsl:if>
