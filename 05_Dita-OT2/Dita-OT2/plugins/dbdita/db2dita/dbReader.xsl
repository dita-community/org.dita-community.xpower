...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="*[@condition or @conformance or @security]" mode="otherprops.att.in">
        <xsl:param name="isRequired"/>
...
<xsl:template match="*[@status]" mode="status.att.in">
  ...
  <xsl:apply-templates select="." mode="status.att.out">
    ...
    <xsl:template
        match="/|*[appendix or article or bibliography or book or
    ...
    <xsl:template match="*[not(title)]" mode="topicId">
        <xsl:call-template name="makeTopicId">
            <xsl:with-param name="titleElement" select="*[position() = 1 and contains(local-name(), 'info')]/title"/>
        </xsl:call-template>
    </xsl:template>

    <xsl:template
        match="
            colophon[not(title)] | dedication[not(title)] |
    ...
    <xsl:template match="colophon | dedication | msgset | msgentry | partintro | simplemsgentry"
    ...
    <xsl:template
        match="
            refentry[not(refsynopsisdiv/refsect2) and
            not(refsect1/refsect2) and not(refsection/refsection)]"
        mode="topic.topic.out">
        <xsl:apply-templates select="." mode="reference.reference.out"/>
    </xsl:template>

    <xsl:template match="refentry[not(refentryinfo/title) and refmeta/refentrytitle]" mode="topic.title.in">
        <xsl:param name="container"/>
        <xsl:param name="isRequired"/>
    ...
    <xsl:template match="
            refentry[not(refentryinfo/title) and
    ...
    <xsl:template match="refentry" mode="reference.refbody.content.in">
        <xsl:apply-templates select="refsynopsisdiv | refsect1 | refsection" mode="reference.refbody.child"/>
    </xsl:template>
    ...
    <xsl:template match="refentry" mode="topic.topic.topic.topic.in">
        <xsl:apply-templates select="refsynopsisdiv | refsect1 | refsection" mode="topic.topic.out"/>
    </xsl:template>
    ...
    <xsl:template match="refentry" mode="topic.shortdesc.in">
        <xsl:param name="container"/>
        <xsl:param name="isRequired"/>
        <xsl:apply-templates select="refnamediv/refpurpose" mode="topic.shortdesc.out"/>
    </xsl:template>
    ...
    <xsl:template match="*[*[position() = 1 and contains(local-name(), 'info')]]" mode="topic.prolog.in">
        <xsl:param name="container"/>
        <xsl:param name="isRequired"/>
    ...
    <xsl:template match="*[position() = 1 and contains(local-name(), 'info')]" mode="topic.metadata.in">
    ...
    <xsl:template match="*" mode="topic.body.content.in">
        <xsl:apply-templates
            select="
                ackno | address | anchor | annotation |
    ...
    <xsl:template match="msgentry" mode="topic.body.content.in">
        <xsl:apply-templates select="msg/msgmain | msg/msgsub | msg/msgrel | msginfo | msgexplan"
    ...
    <xsl:template
        match="
            task[following-sibling::ackno or
    ...
    <xsl:template
        match="
            task[not(following-sibling::ackno or
            following-sibling::address or
            following-sibling::annotation or following-sibling::bibliolist or
            following-sibling::blockquote or following-sibling::bridgehead or
            following-sibling::calloutlist or following-sibling::caution or
    ...
    <xsl:template
        match="
            qandaset[following-sibling::ackno or
            following-sibling::address or
            following-sibling::annotation or following-sibling::bibliolist or
            following-sibling::blockquote or following-sibling::bridgehead or
            following-sibling::calloutlist or following-sibling::caution or
    ...
    <xsl:template
        match="
            qandaset[not(following-sibling::ackno or
            following-sibling::address or
            following-sibling::annotation or following-sibling::bibliolist or
            following-sibling::blockquote or following-sibling::bridgehead or
            following-sibling::calloutlist or following-sibling::caution or
    ...
    <xsl:template
        match="
            refsynopsisdiv[refsect2 and not(refsect2/refsect3)] |
    ...
    <xsl:template match="refsynopsisdiv | refsect1 | refsect2 | refsection" mode="reference.refbody.in">
        <xsl:apply-templates select="." mode="reference.refbody.out"/>
    </xsl:template>
    ...
    <xsl:template match="refsection[refsection and not(refsection/refsection)]" mode="reference.refbody.content.in">
        <xsl:apply-templates select="refsection" mode="reference.refbody.child"/>
    </xsl:template>

    <xsl:template match="refsynopsisdiv | refsect1 | refsect2 | refsection" mode="reference.reference.topic.topic.in"/>

    <xsl:template match="refsynopsisdiv | refsect1" mode="topic.topic.topic.topic.in">
    ...
    <xsl:template match="refsect1 | refsect2 | refsect3 | refsection" mode="reference.refbody.child">
        <xsl:apply-templates select="." mode="topic.section.out"/>
    </xsl:template>
    ...
    <xsl:template match="glossentry[indexterm or revhistory]" mode="topic.prolog.in">
        <xsl:param name="container"/>
        <xsl:param name="isRequired" select="'no'"/>
    ...
    <xsl:template match="glosssee[not(@otherterm)]" mode="topic.related-links.child">
        <xsl:param name="container"/>
        <xsl:param name="isRequired" select="'no'"/>
    ...
    <xsl:template match="itemizedlist | stepalternatives" mode="child">
        <xsl:apply-templates select="*[local-name() != 'listitem' and local-name() != 'step']" mode="child"/>
    ...
    <xsl:template match="*[itemizedlist or stepalternatives]" mode="topic.ul.in">
        <xsl:param name="container"/>
        <xsl:param name="isRequired"/>
        <xsl:apply-templates select="itemizedlist | stepalternatives" mode="topic.ul.out"/>
    </xsl:template>
    ...
    <xsl:template match="*[orderedlist or procedure or substeps]" mode="topic.ol.in">
        <xsl:param name="container"/>
        <xsl:param name="isRequired"/>
        <xsl:apply-templates select="orderedlist | procedure | substeps" mode="topic.ol.out"/>
    </xsl:template>
    ...
    <xsl:template match="step[not(title)]" mode="task.step.content.in">
        <xsl:apply-templates select=". | substeps | stepalternatives" mode="task.step.child"/>
    </xsl:template>

    <xsl:template match="step[title]" mode="task.step.content.in">
        <xsl:apply-templates select="title | substeps | stepalternatives" mode="task.step.child"/>
    </xsl:template>

    <xsl:template match="step" mode="task.step.child">
        <xsl:variable name="firstInfo"
            select="
                *[
    ...
    <xsl:template match="*[ackno or formalpara or para or simpara]" mode="topic.p.in">
        <xsl:param name="container"/>
        <xsl:param name="isRequired"/>
        <xsl:apply-templates select="ackno | formalpara | para | simpara" mode="topic.p.out"/>
    </xsl:template>
    ...
    <xsl:template match="*[caution or important or note or tip or warning]" mode="topic.note.in">
        <xsl:param name="container"/>
        <xsl:param name="isRequired"/>
        <xsl:apply-templates select="caution | important | note | tip | warning" mode="topic.note.out"/>
    </xsl:template>
    ...
    <xsl:template
        match="
            address | literallayout[@class = 'monospaced'] |
    ...
    <xsl:template
        match="
            *[address or literallayout[@class = 'monospaced'] or
    ...
    <xsl:template match="literallayout[not(@class) or @class != 'monospaced']" mode="child">
        <xsl:apply-templates select="." mode="topic.lines.out"/>
    </xsl:template>

    <xsl:template match="*[literallayout[not(@class) or @class != 'monospaced']]" mode="topic.lines.in">
        <xsl:param name="container"/>
        <xsl:param name="isRequired"/>
        <xsl:apply-templates select="literallayout[not(@class) or @class != 'monospaced']" mode="topic.lines.out"/>
    </xsl:template>
    ...
    <xsl:template match="emphasis[not(@role) or @role = 'em' or @role = 'italic' or @role = 'i']" mode="child">
        <xsl:apply-templates select="." mode="hi-d.i.out"/>
    </xsl:template>

    <xsl:template
        match="
            *[emphasis[not(@role) or @role = 'em' or
    ...
    <xsl:template match="emphasis[@role = 'strong' or @role = 'bold' or @role = 'b']" mode="child">
        <xsl:apply-templates select="." mode="hi-d.b.out"/>
    </xsl:template>

    <xsl:template match="*[emphasis[@role = 'strong' or @role = 'bold' or @role = 'b']]" mode="hi-d.b.in">
        <xsl:param name="container"/>
        <xsl:param name="isRequired"/>
        <xsl:apply-templates select="emphasis[@role = 'strong' or @role = 'bold' or @role = 'b']" mode="hi-d.b.out"/>
    </xsl:template>

    <xsl:template match="emphasis[@role = 'underline' or @role = 'underscore' or @role = 'u']" mode="child">
        <xsl:apply-templates select="." mode="hi-d.u.out"/>
    </xsl:template>

    <xsl:template match="*[emphasis[@role = 'underline' or @role = 'underscore' or @role = 'u']]" mode="hi-d.u.in">
        <xsl:param name="container"/>
        <xsl:param name="isRequired"/>
        <xsl:apply-templates select="emphasis[@role = 'underline' or @role = 'underscore' or @role = 'u']"
            mode="hi-d.u.out"/>
    </xsl:template>
    ...
    <xsl:template
        match="
            emphasis[@role = 'monospace' or @role = 'mono' or
    ...
    <xsl:template
        match="
            abbrev | acronym | action | authorinitials | citebiblioid |
    ...
    <xsl:template
        match="
            *[author or abbrev or acronym or action or
        ...
        <xsl:apply-templates
            select="
                author | abbrev | acronym | action | authorinitials |
    ...
    <xsl:template match="*[code or initializer or literal or returnvalue]" mode="pr-d.codeph.in">
        <xsl:param name="container"/>
        <xsl:param name="isRequired"/>
        <xsl:apply-templates select="code | initializer | literal | returnvalue" mode="pr-d.codeph.out"/>
    </xsl:template>
    ...
    <xsl:template match="*[computeroutput or prompt]" mode="sw-d.systemoutput.in">
        <xsl:param name="container"/>
        <xsl:param name="isRequired"/>
        <xsl:apply-templates select="computeroutput | prompt" mode="sw-d.systemoutput.out"/>
    </xsl:template>
    ...
    <xsl:template match="menuchoice" mode="ui-d.menucascade.content.in">
        <xsl:apply-templates select="*[position() != last()]" mode="ui-d.uicontrol.out"/>
    ...
    <xsl:template match="menuchoice" mode="ui-d.uicontrol.content.in">
        ...
        <xsl:apply-templates select="*[position() = last()]" mode="ui-d.uicontrol.child"/>
    </xsl:template>

    <xsl:template match="guibutton | guilabel | guimenu | guimenuitem | guisubmenu | interface" mode="child">
        <xsl:apply-templates select="." mode="ui-d.uicontrol.out"/>
    </xsl:template>

    <xsl:template match="guibutton | guilabel | guimenu | guimenuitem | guisubmenu | interface"
    ...
    <xsl:template
        match="
            *[guibutton or guilabel or guimenu or guimenuitem or
            guisubmenu or interface]"
        mode="ui-d.uicontrol.in">
        <xsl:param name="container"/>
        <xsl:param name="isRequired"/>
        <xsl:apply-templates select="guibutton | guilabel | guimenu | guimenuitem | guisubmenu | interface"
    ...
    <xsl:template
        match="
            classname | constant | exceptionname | funcparams | function |
    ...
    <xsl:template
        match="
            *[classname or constant or exceptionname or
        ...
        <xsl:apply-templates
            select="
                classname | constant | exceptionname | funcparams |
    ...
    <xsl:template match="*[database or envar or property or replaceable or varname]" mode="sw-d.varname.in">
        <xsl:param name="container"/>
        <xsl:param name="isRequired"/>
        <xsl:apply-templates select="database | envar | property | replaceable | varname" mode="sw-d.varname.out"/>
    </xsl:template>
    ...
    <xsl:template match="*[citation or citetitle or citerefentry]" mode="topic.cite.in">
        <xsl:param name="container"/>
        <xsl:param name="isRequired"/>
        <xsl:apply-templates select="citation | citetitle | citerefentry" mode="topic.cite.out"/>
    </xsl:template>
    ...
    <xsl:template match="firstterm | glossterm[not(parent::glossentry)] | wordasword" mode="child">
    ...
    <xsl:template match="*[firstterm or glossterm[not(parent::glossentry)]]" mode="topic.term.in">
        <xsl:param name="container"/>
        <xsl:param name="isRequired"/>
        <xsl:apply-templates select="firstterm | glossterm[not(parent::glossentry)]" mode="topic.term.out"/>
    </xsl:template>
    ...
    <xsl:template match="primary" mode="topic.indexterm.content.in">
        ...
        <xsl:apply-templates select="following-sibling::secondary" mode="topic.indexterm.out"/>
    </xsl:template>

    <xsl:template match="secondary" mode="topic.indexterm.content.in">
        <xsl:apply-templates mode="topic.indexterm.child"/>
        <xsl:apply-templates select="following-sibling::tertiary" mode="topic.indexterm.out"/>
    </xsl:template>

    <xsl:template match="*[indexterm[not(@zone)]]" mode="topic.indexterm.in">
        <xsl:param name="container"/>
        <xsl:param name="isRequired"/>
        <xsl:apply-templates select="indexterm[not(@zone)]" mode="topic.indexterm.out"/>
    </xsl:template>
    ...
    <xsl:template match="biblioref[@linkend] | link[@linkend] | xref[@linkend]" mode="href.att.in">
        <xsl:param name="isRequired"/>
        <xsl:call-template name="makeRef"/>
    </xsl:template>

    <xsl:template match="link[@endterm and not(* or text())]" mode="topic.xref.content.in">
        <xsl:param name="isRequired"/>
    ...
    <xsl:template match="*[biblioref or link or ulink or xref]" mode="topic.xref.in">
        <xsl:param name="container"/>
        <xsl:param name="isRequired"/>
        <xsl:apply-templates select="biblioreflink | ulink | xref" mode="topic.xref.out"/>
    </xsl:template>
    ...
    <xsl:template match="informalexample | informalfigure" mode="topic.example.child">
    ...
    <xsl:template match="*[example or informalexample]" mode="topic.example.in">
        <xsl:param name="container"/>
        <xsl:param name="isRequired"/>
        <xsl:apply-templates select="example | informalexample" mode="topic.example.out"/>
    </xsl:template>

    <xsl:template
        match="
            equation | example | figure |
            informalequation | informalexample | informalfigure | screenshot | sidebar"
        mode="child">
        <xsl:apply-templates select="." mode="topic.fig.out"/>
    </xsl:template>

    <xsl:template match="equation/title | example/title | figure/title | sidebar/title" mode="child">
        <xsl:apply-templates select="." mode="topic.title.out"/>
    </xsl:template>
    ...
    <xsl:template match="blockinfo[not(title)] | sidebarinfo[not(title)]" mode="child"/>
    ...
    <xsl:template
        match="
            *[equation or example or figure or informalequation or
    ...
    <xsl:template
        match="
            classsynopsis | cmdsynopsis | constructorsynopsis |
    ...
    <xsl:template
        match="
            *[classsynopsis or cmdsynopsis or constructorsynopsis or
    ...
    <xsl:template match="sbr" mode="pr-d.groupcomp.content.in">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:apply-templates select="following-sibling::node()" mode="pr-d.groupcomp.child"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template match="arg[@choice = 'req'] | group[@choice = 'req']" mode="pr-d.groupchoice.importance.att.in">
        <xsl:param name="isRequired"/>
    ...
    <xsl:template match="classsynopsis" mode="pr-d.syntaxdiagram.content.in">
        <xsl:apply-templates select="ooclass | oointerface | ooexception" mode="pr-d.groupchoice.out"/>
        <xsl:apply-templates
            select="
    ...
    <xsl:template
        match="
            classname | exceptionname | funcdef | initializer | interfacename |
    ...
    <xsl:template match="co[@id and not(@label)]" mode="topic.keyword.content.in">
        <xsl:variable name="containerId" select="generate-id(ancestor::*[.//co and count(.//co) > 1][1])"/>
        <xsl:value-of
            select="
                count(. |
    ...
    <xsl:template
        match="
            graphic[@fileref] | imageobject[imagedata/@fileref] |
    ...
    <xsl:template match="graphic | mediaobject/imageobject/imagedata" mode="placement.att.in">
        <xsl:param name="isRequired"/>
    ...
    <xsl:template match="graphic[@fileref] | imagedata[@fileref] | inlinegraphic[@fileref]" mode="href.att.in">
        <xsl:param name="isRequired"/>
        <xsl:apply-templates select="." mode="href.att.out">
    ...
    <xsl:template
        match="
            graphic[@entityref] | imagedata[@entityref] |
    ...
    <xsl:template
        match="
            *[graphic or imageobject or inlinegraphic or
        ...
        <xsl:apply-templates
            select="
                graphic | imageobject | inlinegraphic |
    ...
    <xsl:template match="imageobject" mode="topic.alt.in">
        <xsl:param name="container"/>
        <xsl:param name="isRequired" select="'no'"/>
        <xsl:apply-templates select="following-sibling::textobject[1]" mode="topic.alt.out"/>
    </xsl:template>
    ...
    <xsl:template match="mediaobject | inlinemediaobject" mode="topic.object.atts.in">
        <xsl:apply-templates select="audioobject | videoobject" mode="topic.object.atts.in"/>
    </xsl:template>

    <xsl:template match="mediaobject[caption] | inlinemediaobject[caption]" mode="topic.desc.in">
        <xsl:apply-templates select="caption" mode="topic.desc.out"/>
    </xsl:template>

    <xsl:template
        match="
            mediaobject[not(caption) and not(objectinfo)] |
            inlinemediaobject[not(caption) and not(objectinfo)]"
        mode="topic.desc.in">
        <xsl:apply-templates select="audioobject | imageobject | videoobject" mode="topic.desc.in"/>
    </xsl:template>
    ...
    <xsl:template match="audiodata[@fileref] | videodata[@fileref]" mode="data.att.in">
        <xsl:param name="isRequired"/>
    ...
    <xsl:template match="*[@depth and not(contains(@depth, '%'))]" mode="height.att.in">
        <xsl:param name="isRequired"/>
    ...
    <xsl:template match="*[@width and not(contains(@width, '%'))]" mode="width.att.in">
        <xsl:param name="isRequired"/>
    ...
    <xsl:template match="table[not(tgroup)] | informaltable[not(tgroup)]" mode="topic.tgroup.in">
        <xsl:apply-templates select="." mode="topic.tgroup.out"/>
    </xsl:template>

    <xsl:template match="table | informaltable" mode="topic.tgroup.id.atts.in"/>

    <xsl:template match="table[col or colgroup] | informaltable[col or colgroup]" mode="topic.colspec.in">
    ...
    <xsl:template match="tbody" mode="topic.tbody.content.in">
        ...
        <xsl:apply-templates select="preceding-sibling::tfoot" mode="topic.tbody.topic.row.in"/>
    </xsl:template>

    <xsl:template match="table[not(tbody)] | informaltable[not(tbody)]" mode="topic.tbody.in">
        <xsl:apply-templates select="." mode="topic.tbody.out"/>
    </xsl:template>
...
FOR ANYTHING THAT CAN APPEAR IN *info or *meta
 ...
 -->
    ...
    <xsl:template match="author[not(email) and address/email[1]]" mode="topic.author.href.att.in">
        <xsl:param name="isRequired"/>
        <xsl:apply-templates select="address/email[1]" mode="topic.author.href.att.in">
            <xsl:with-param name="isRequired" select="$isRequired"/>
        </xsl:apply-templates>
    </xsl:template>
    ...
    <xsl:template
        match="
            author[not(email) and not(address/email) and
            affiliation/address/email[1]]"
        mode="topic.author.href.att.in">
        <xsl:param name="isRequired"/>
        <xsl:apply-templates select="affiliation/address/email[1]" mode="topic.author.href.att.in">
            <xsl:with-param name="isRequired" select="$isRequired"/>
        </xsl:apply-templates>
    </xsl:template>
    ...
    <xsl:template match="author" mode="words.cnt.text.in">
        <xsl:apply-templates select="personname | honorific | firstname | surname | lineage | othername"
            mode="words.cnt.text.in"/>
    </xsl:template>

    <xsl:template match="personname" mode="words.cnt.text.in">
        <xsl:apply-templates select="honorific | firstname | surname | lineage | othername" mode="words.cnt.text.in"/>
    </xsl:template>

    <xsl:template match="honorific | firstname | surname | lineage | othername" mode="words.cnt.text.in">
        <xsl:if
            test="
                preceding-sibling::honorific or
    ...
    <xsl:template match="*[publisher]" mode="topic.publisher.in">
        <xsl:param name="container"/>
        <xsl:param name="isRequired"/>
        <xsl:apply-templates select="publisher/publishername" mode="topic.publisher.out"/>
    </xsl:template>
    ...
    <xsl:template match="copyright[not(holder)]" mode="topic.copyrholder.content.in"/>
    ...
    <xsl:template match="revhistory" mode="topic.critdates.content.in">
        ...
        <xsl:apply-templates select="revision[position() > 1]/date" mode="topic.revised.out"/>
    </xsl:template>
    ...
    <xsl:template match="*[not(itermset) and not(keywordset)]" mode="topic.keywords.in">
        <xsl:if
            test="
                indexterm or (position() = 1 and
    ...
    <xsl:template match="*[local-name() != 'itermset' and local-name() != 'keywordset']"
    ...
    <xsl:template match="*[productname or productnumber]" mode="topic.prodinfo.content.in">
        <xsl:choose>
    ...
    <xsl:template match="titleabbrev | index | setindex | lot | toc | tocchap | beginpage" mode="child"/>
    ...
    <xsl:template name="camelCase">
        ...
        <xsl:call-template name="camelCasePart">
            <xsl:with-param name="input"
                select="
                    translate(normalize-space($input),
    ...
    <xsl:template name="camelCasePart">
        ...
        <xsl:choose>
            <xsl:when test="contains($input, $delimiter)">
                <xsl:variable name="beforePart" select="substring-before($input, $delimiter)"/>
                <xsl:variable name="afterPart" select="substring-after($input, $delimiter)"/>
